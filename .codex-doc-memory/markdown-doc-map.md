# Markdown Documentation Map

Generated: 2026-08-20T11:23:47.199Z
Root: C:\Users\Administrator\Desktop\ivr

## Summary

- Markdown files: 526
- Links resolved: 478
- Unresolved links: 39
- Duplicate titles: 1
- Encoding/name anomalies: 16
- Orphan candidates: 111

## High Link Documents

| File | Title | Incoming | Outgoing | Words |
| --- | --- | ---: | ---: | ---: |
| docs/documents/00-DOC-READING-ORDER.md | Documentation Reading Order | 0 | 177 | 4269 |
| prompt/00-index.md | IVR Prompt Library — Master Index | 1 | 59 | 783 |
| plan/ivr-orther/decisions-log.md | Decisions Log — IVR (lịch sử quyết định + Target V1 overlay) | 16 | 3 | 3478 |
| docs/contracts/telephony-procurement-pack/R-01-vendor-requirements.md | R-01 — Yêu cầu nhà cung cấp telephony | 13 | 4 | 2363 |
| docs/contracts/telephony-procurement-pack/README.md | Telephony Procurement Pack — 1 SIM lab và 32 eSIM production | 0 | 17 | 863 |
| plan/ivr-orther/_archive/prompts/00-index.md | Prompts Index — IVR-Orther | 1 | 16 | 572 |
| docs/contracts/target-v1-closure-pack/README.md | Target Contract V1 — Closure Pack | 1 | 11 | 968 |
| docs/contracts/telephony-procurement-pack/R-05-tts-audio-capability.md | R-05 — Năng lực TTS và nguồn audio | 7 | 5 | 1441 |
| docs/contracts/telephony-procurement-pack/R-04-scorecard-and-gaps.md | R-04 — Scorecard nhà cung cấp, gap register và điều khoản hợp đồng | 2 | 9 | 1403 |
| prompt/_legacy-mock/00-index.md | IVR Dev Prompt Library — Index | 0 | 9 | 369 |
| specs/testing/00-index.md | Testing SRS — Index | 0 | 9 | 122 |
| specs/workflows/00-index.md | Workflows — Index | 0 | 9 | 119 |
| docs/contracts/target-v1-closure-pack/T-08-openapi-compat-cdc.md | T-08 — OpenAPI compatibility, deprecation và sở hữu consumer-driven test | 3 | 5 | 886 |
| docs/contracts/telephony-procurement-pack/R-02-lab-package.md | R-02 — Gói lab 1 SIM thật | 3 | 5 | 1612 |
| docs/contracts/telephony-procurement-pack/R-03-esim32-package.md | R-03 — Gói production nhiều kênh eSIM | 4 | 4 | 1687 |
| specs/api/00-index.md | API SRS — Index | 0 | 8 | 160 |
| specs/functional/00-index.md | Functional SRS — Index | 0 | 8 | 379 |
| specs/ui/00-index.md | UI SRS — Index (Admin/Ops Console) | 0 | 8 | 247 |
| docs/contracts/target-v1-closure-pack/T-03-speech-summary.md | T-03 — Privacy-safe order summary và whitelist biến lời thoại | 3 | 4 | 1042 |
| docs/contracts/telephony-procurement-pack/lab-acceptance-report-template.md | Biểu mẫu — Báo cáo nghiệm thu lab 1 SIM | 5 | 2 | 1252 |
| plan/ivr-orther/_archive/16-prompt-roadmap.md | 16 — Prompt Roadmap | 1 | 6 | 1036 |
| specs/architecture/00-index.md | Architecture SRS — Index | 0 | 7 | 98 |
| docs/contracts/target-v1-closure-pack/T-02-task-data-order-version.md | T-02 — Task data: `order_version`, `order_state`, eligibility/restriction evidence | 3 | 3 | 866 |
| docs/contracts/target-v1-closure-pack/T-04-dial-token.md | T-04 — Dial-token: issue / resolve / TTL / one-use / audit | 4 | 2 | 978 |
| docs/contracts/target-v1-closure-pack/T-07-production-auth.md | T-07 — Production service auth: JWT issuer/audience/scope/TTL/JWKS + mTLS | 5 | 1 | 966 |

## Unresolved Links

| File | Line | Target | Kind |
| --- | ---: | --- | --- |
| docs/contracts/target-v1-closure-pack/README.md | 51 | ../../../specs/api/openapi/contract-manifest.json | markdown |
| docs/contracts/target-v1-closure-pack/T-01-program-matrix.md | 19 | ../../../specs/api/openapi/ivr-order-confirmation.v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-01-program-matrix.md | 20 | ../../../src/Ivr.Api/Intake/TaskIntakeEndpoint.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-01-program-matrix.md | 21 | ../../../src/Ivr.Domain/Policies/EligibilityRules.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-01-program-matrix.md | 66 | ../../../tests/Ivr.UnitTests/Intake/TaskIntakeServiceTests.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-02-task-data-order-version.md | 15 | ../../../specs/api/openapi/ivr-order-confirmation.v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-02-task-data-order-version.md | 16 | ../../../specs/api/openapi/order-core-ivr-callback.target-v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-02-task-data-order-version.md | 25 | ../../../src/Ivr.Domain/Policies/EligibilityRules.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-02-task-data-order-version.md | 37 | ../../../src/Ivr.Api/Intake/TaskIntakeEndpoint.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-02-task-data-order-version.md | 50 | ../../../src/Ivr.Domain/Policies/EligibilityRules.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-03-speech-summary.md | 11 | ../../../specs/api/openapi/ivr-order-confirmation.v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-03-speech-summary.md | 32 | ../../../src/Ivr.Domain/Confirmation/PrivacySafeSpeech.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-03-speech-summary.md | 81 | ../../../tests/Ivr.UnitTests/Intake/TaskIntakeServiceTests.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-04-dial-token.md | 11 | ../../../specs/api/openapi/ivr-order-confirmation.v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-04-dial-token.md | 31 | ../../../src/Ivr.Domain/Ports/ProviderPorts.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-04-dial-token.md | 91 | ../../../deploy/ci/scripts/scan-pii.sh | markdown |
| docs/contracts/target-v1-closure-pack/T-05-callback-ack.md | 11 | ../../../specs/api/openapi/order-core-ivr-callback.target-v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-05-callback-ack.md | 25 | ../../../src/Ivr.Infrastructure/Callbacks/TargetV1CallbackTransport.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-05-callback-ack.md | 33 | ../../../specs/api/compat/current-golden-hour-callback.a3aad246.schema.json | markdown |
| docs/contracts/target-v1-closure-pack/T-05-callback-ack.md | 92 | ../../../tests/Ivr.ContractTests/SalesContractScaffoldTests.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-06-no-answer-timeout.md | 17 | ../../../specs/api/openapi/order-core-ivr-callback.target-v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-06-no-answer-timeout.md | 28 | ../../../src/Ivr.Api/Application/AdminReadService.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-06-no-answer-timeout.md | 28 | ../../../src/Ivr.Api/Application/AnalyticsReadService.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-07-production-auth.md | 17 | ../../../src/Ivr.Api/Auth/OrderCoreAllowlistMiddleware.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-07-production-auth.md | 18 | ../../../src/Ivr.Api/Internal/InternalServiceOptions.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-07-production-auth.md | 19 | ../../../src/Ivr.Api/Auth/MockPermissionAuthenticationHandler.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-07-production-auth.md | 20 | ../../../src/Ivr.Domain/Ports/ProviderPorts.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-07-production-auth.md | 26 | ../../../specs/api/openapi/order-core-ivr-callback.target-v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-07-production-auth.md | 40 | ../../../specs/api/openapi/ivr-order-confirmation.v1.yaml | markdown |
| docs/contracts/target-v1-closure-pack/T-08-openapi-compat-cdc.md | 15 | ../../../specs/api/openapi/contract-manifest.json | markdown |
| docs/contracts/target-v1-closure-pack/T-08-openapi-compat-cdc.md | 16 | ../../../deploy/ci/docs.gitlab-ci.yml | markdown |
| docs/contracts/target-v1-closure-pack/T-08-openapi-compat-cdc.md | 23 | ../../../specs/api/compat/current-golden-hour-callback.a3aad246.schema.json | markdown |
| docs/contracts/target-v1-closure-pack/T-09-attempt-policy.md | 11 | ../../../src/Ivr.Infrastructure/Intake/AttemptPolicyRegistries.cs | markdown |
| docs/contracts/target-v1-closure-pack/T-09-attempt-policy.md | 83 | ../../../tests/Ivr.UnitTests/Intake/TaskIntakeServiceTests.cs | markdown |
| docs/contracts/telephony-procurement-pack/R-01-vendor-requirements.md | 13 | ../../../src/Ivr.Domain/Ports/ProviderPorts.cs | markdown |
| docs/contracts/telephony-procurement-pack/R-01-vendor-requirements.md | 96 | ../../../src/Ivr.Domain/Ports/ProviderPorts.cs | markdown |
| docs/evidence/W-0026/README.md | 84 | live-screens.txt | markdown |
| docs/evidence/W-0030/README.md | 24 | ../../../specs/api/evidence/eligibility-snapshot.v1.schema.json | markdown |
| docs/evidence/W-0097/README.md | 119 | design-preview.html | markdown |

## Duplicate Titles

- gitnexus — code intelligence: AGENTS.md, CLAUDE.md

## Encoding Or Name Anomalies

- docs/documents/4. phase/phase-1/00-PHÂN TÍCH NGUỒN SỰ THẬT VÀ PHẠM VI.md
- docs/documents/4. phase/phase-1/10-DANH MỤC PHÂN NHÓM NGUYÊN LIỆU VÀ VẬT TƯ.md
- docs/documents/4. phase/phase-2/00-PHÂN TÍCH VẬN HÀNH LÕI.md
- docs/documents/4. phase/phase-3/00-PHÂN TÍCH HIỆN TRẠNG.md
- docs/documents/4. phase/phase-3/05-ĐƠN CHÍNH THỨC MÃ ĐƠN VÀ MÁY TRẠNG THÁI.md
- docs/documents/4. phase/phase-3.1/00-PHÂN TÍCH HIỆN TRẠNG GIAI ĐOẠN 3.1.md
- docs/documents/4. phase/phase-4/00-PHÂN TÍCH HIỆN TRẠNG.md
- docs/documents/4. phase/phase-5/00-PHÂN TÍCH HIỆN TRẠNG.md
- docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/04-MÔ HÌNH GIÁ GIỜ VÀNG TRI ÂN DOANH THU BÁN HÀNG TRỰC TIẾP QUẢNG CÁO VÀ DIAMOND.md
- docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/07-PHÂN TÍCH MÔ HÌNH TÀI CHÍNH.md
- docs/documents/4. phase/phase-6/00-PHÂN TÍCH HIỆN TRẠNG.md
- docs/documents/4. phase/phase-6/02-PHÂN LOẠI SỰ KIỆN VÀ TÍN HIỆU PHỄU BÁN HÀNG.md
- docs/documents/4. phase/phase-6/03-PIXEL CAPI CHỐNG TRÙNG VÀ PHÂN BỔ GHI NHẬN.md
- docs/documents/4. phase/phase-6/04-DOANH THU ĐÃ XÁC THỰC VÀ QUY TẮC LOẠI TRỪ.md
- docs/documents/4. phase/phase-7/00-PHÂN TÍCH HIỆN TRẠNG.md
- docs/documents/4. phase/phase-7/04-PHÂN LOẠI BÌNH LUẬN BÁN HÀNG TRỰC TIẾP VÀ ĐIỀU PHỐI MESSENGER.md

## Orphan Candidates

- .claude/skills/gitnexus/gitnexus-cli/SKILL.md
- .claude/skills/gitnexus/gitnexus-debugging/SKILL.md
- .claude/skills/gitnexus/gitnexus-exploring/SKILL.md
- .claude/skills/gitnexus/gitnexus-guide/SKILL.md
- .claude/skills/gitnexus/gitnexus-impact-analysis/SKILL.md
- .claude/skills/gitnexus/gitnexus-refactoring/SKILL.md
- admin-ui/AGENTS.md
- admin-ui/CLAUDE.md
- admin-ui/README.md
- AGENTS.md
- CLAUDE.md
- deploy/chaos/README.md
- deploy/dr/README.md
- deploy/lab/README.md
- docs/compliance/data-inventory.md
- docs/compliance/dsar-runbook.md
- docs/compliance/ivr-data-inventory.md
- docs/compliance/ivr-pdpa-legal-basis-pack.md
- docs/compliance/ivr-retention-options.md
- docs/compliance/pia.md
- docs/compliance/release-compliance-checklist.md
- docs/compliance/retention.md
- docs/contracts/openapi-codegen.md
- docs/contracts/openapi-contract-diff.md
- docs/cost-model.md
- docs/data-governance.md
- docs/dr-topology.md
- docs/evidence/W-0010/README.md
- docs/evidence/W-0011/README.md
- docs/evidence/W-0012/README.md
- docs/evidence/W-0013/README.md
- docs/evidence/W-0014/README.md
- docs/evidence/W-0015/README.md
- docs/evidence/W-0016/README.md
- docs/evidence/W-0017/README.md
- docs/evidence/W-0018/README.md
- docs/evidence/W-0019/README.md
- docs/evidence/W-0020/README.md
- docs/evidence/W-0021/README.md
- docs/evidence/W-0022/README.md
- docs/evidence/W-0023/README.md
- docs/evidence/W-0025/README.md
- docs/evidence/W-0026/README.md
- docs/evidence/W-0027/README.md
- docs/evidence/W-0028/README.md
- docs/evidence/W-0029/README.md
- docs/evidence/W-0031/README.md
- docs/evidence/W-0033/README.md
- docs/evidence/W-0034/README.md
- docs/evidence/W-0035/README.md
- docs/evidence/W-0036/README.md
- docs/evidence/W-0038/README.md
- docs/evidence/W-0039/README.md
- docs/evidence/W-0040/README.md
- docs/evidence/W-0041/README.md
- docs/evidence/W-0045/README.md
- docs/evidence/W-0048/external-input-request.md
- docs/evidence/W-0048/README.md
- docs/evidence/W-0052/README.md
- docs/evidence/W-0053/README.md
- docs/evidence/W-0054/README.md
- docs/evidence/W-0055/README.md
- docs/evidence/W-0057/README.md
- docs/evidence/W-0058/README.md
- docs/evidence/W-0059/README.md
- docs/evidence/W-0060/README.md
- docs/evidence/W-0061/README.md
- docs/evidence/W-0065/README.md
- docs/evidence/W-0085/README.md
- docs/evidence/W-0086/README.md
- docs/evidence/W-0087/README.md
- docs/evidence/W-0088/README.md
- docs/evidence/W-0089/README.md
- docs/evidence/W-0090/README.md
- docs/evidence/W-0091/README.md
- docs/evidence/W-0092/README.md
- docs/evidence/W-0093/README.md
- docs/evidence/W-0094/README.md
- docs/evidence/W-0095/README.md
- docs/evidence/W-0096/README.md
- docs/evidence/W-0097/README.md
- docs/evidence/W-0098/README.md
- docs/evidence/W-0099/README.md
- docs/evidence/W-0100/README.md
- docs/evidence/W-0101/README.md
- docs/evidence/W-0102/README.md
- docs/evidence/W-0104/README.md
- docs/integration-guide.md
- docs/kpi-catalog.md
- docs/lab/one-sim-lab-plan.md
- docs/operations/gitlab-runner-winhost.md
- docs/owner-decisions-open.md
- docs/release/df03-signoff-input.md
- docs/release/readiness-board.md
- docs/reports/2026-08-15-bao-cao-tien-do-ivr.md
- docs/review/2026-08-14-phase-1-2-code-review.md
- docs/slo.md
- docs/traceability-tests.md
- plan/ivr-orther/_archive/01-reading-inventory.md
- plan/ivr-orther/_archive/08-target-specs-structure-proposal.md
- prompt/_review/phase-0-11-spec-alignment-review.md
- prompt/_review/zero-to-production-prompt-readiness-review.md
- prompt/_TEMPLATE.md
- seed/README.md
- specs/_review/normalization-report.md
- specs/_review/traceability-matrix.md
- specs/03-stakeholders-and-actors.md
- specs/05-current-docs-review.md
- specs/decisions/DF-07-retention-policy.md
- specs/decisions/DT-05-recording-off-policy.md
- specs/tech/00-tech-stack.md

## File Inventory

| File | Title | H1/Headings | Incoming | Outgoing | Unresolved | Words |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| .claude/skills/gitnexus/gitnexus-cli/SKILL.md | GitNexus CLI Commands | 1/9 | 0 | 0 | 0 | 581 |
| .claude/skills/gitnexus/gitnexus-debugging/SKILL.md | Debugging with GitNexus | 1/7 | 0 | 0 | 0 | 264 |
| .claude/skills/gitnexus/gitnexus-exploring/SKILL.md | Exploring Codebases with GitNexus | 1/7 | 0 | 0 | 0 | 178 |
| .claude/skills/gitnexus/gitnexus-guide/SKILL.md | GitNexus Guide | 1/10 | 0 | 0 | 0 | 1015 |
| .claude/skills/gitnexus/gitnexus-impact-analysis/SKILL.md | Impact Analysis with GitNexus | 1/8 | 0 | 0 | 0 | 225 |
| .claude/skills/gitnexus/gitnexus-refactoring/SKILL.md | Refactoring with GitNexus | 1/10 | 0 | 0 | 0 | 187 |
| .gitlab/merge_request_templates/Default.md | IVR change traceability | 1/5 | 1 | 2 | 0 | 250 |
| admin-ui/AGENTS.md | This is NOT the Next.js you know | 1/1 | 0 | 0 | 0 | 95 |
| admin-ui/CLAUDE.md | CLAUDE | 0/0 | 0 | 0 | 0 | 1 |
| admin-ui/README.md | IVR Admin UI | 1/5 | 0 | 0 | 0 | 617 |
| AGENTS.md | GitNexus — Code Intelligence | 1/5 | 0 | 0 | 0 | 381 |
| CLAUDE.md | GitNexus — Code Intelligence | 1/5 | 0 | 0 | 0 | 381 |
| deploy/chaos/README.md | Chaos fault injection — `W-0042` · `P6-3` | 1/5 | 0 | 0 | 0 | 365 |
| deploy/ci/README.md | GitLab CI baseline | 1/8 | 1 | 1 | 0 | 1471 |
| deploy/ci/rollback.md | Rollback — `W-0045` · `P7-3` §8 | 1/6 | 1 | 0 | 0 | 495 |
| deploy/docker/README.md | Container images & dev stack — `W-0043` · `P7-1` | 1/10 | 1 | 0 | 0 | 748 |
| deploy/dr/README.md | DR configuration — `W-0053` · `P10-2` | 1/5 | 0 | 0 | 0 | 257 |
| deploy/helm/README.md | Helm chart — `W-0044` · `P7-2` | 1/9 | 1 | 0 | 0 | 820 |
| deploy/lab/README.md | W-0104 — Free Asterisk + MicroSIP lab | 1/6 | 0 | 0 | 0 | 478 |
| docs/api/changelog/ivr-order-confirmation.md | API Changelog 1.0.0-draft.2 vs. 1.0.0-draft.9 | 1/13 | 1 | 0 | 0 | 75 |
| docs/api/changelog/ivr-order-confirmation.v1.0.0-to-v1.0.0-draft.2.md | API Changelog 1.0.0 vs. 1.0.0-draft.2 | 1/20 | 1 | 0 | 0 | 2694 |
| docs/api/changelog/order-core-ivr-callback.md | API Changelog 1.0.0-draft vs. 1.0.0-draft | 1/1 | 1 | 0 | 0 | 15 |
| docs/api-changelog.md | IVR API Contract Changelog | 1/3 | 0 | 4 | 0 | 681 |
| docs/api-versioning.md | IVR API Versioning and Deprecation Policy | 1/7 | 3 | 0 | 0 | 422 |
| docs/capacity-model.md | IVR Capacity and Cost Model | 2/11 | 5 | 0 | 0 | 1298 |
| docs/compliance/data-inventory.md | Data inventory & legal basis — `W-0052` · `P10-1` | 1/6 | 0 | 0 | 0 | 1172 |
| docs/compliance/dsar-runbook.md | DSAR runbook — `W-0052` · `P10-1` | 1/9 | 0 | 0 | 0 | 688 |
| docs/compliance/ivr-data-inventory.md | IVR data inventory (class level, có chủ sở hữu) — `W-0059` · `P11-3` | 1/6 | 0 | 0 | 0 | 747 |
| docs/compliance/ivr-pdpa-legal-basis-pack.md | PDPA / legal basis pack — `W-0059` · `P11-3` | 1/7 | 0 | 0 | 0 | 922 |
| docs/compliance/ivr-retention-options.md | Retention options (trước khi Legal ký) — `W-0059` · `P11-3` | 1/6 | 0 | 0 | 0 | 800 |
| docs/compliance/pia.md | Privacy Impact Assessment — `W-0052` · `P10-1` | 1/6 | 0 | 0 | 0 | 917 |
| docs/compliance/release-compliance-checklist.md | Release compliance checklist — `W-0052` · `P10-1` → cổng `P9-1` | 1/7 | 0 | 0 | 0 | 704 |
| docs/compliance/retention.md | Retention policy — `W-0052` · `P10-1` · DF-07 | 1/6 | 0 | 0 | 0 | 562 |
| docs/contracts/openapi-codegen.md | OpenAPI Code Generation and Contract Drift Policy | 1/6 | 0 | 0 | 0 | 459 |
| docs/contracts/openapi-contract-diff.md | OpenAPI Contract Baseline and Human-Readable Diff | 1/5 | 0 | 0 | 0 | 458 |
| docs/contracts/target-v1-closure-pack/README.md | Target Contract V1 — Closure Pack | 1/8 | 1 | 11 | 1 | 968 |
| docs/contracts/target-v1-closure-pack/T-01-program-matrix.md | T-01 — Ma trận program / payment / IVR-required / callable | 1/8 | 4 | 1 | 4 | 807 |
| docs/contracts/target-v1-closure-pack/T-02-task-data-order-version.md | T-02 — Task data: `order_version`, `order_state`, eligibility/restriction evidence | 1/8 | 3 | 3 | 5 | 866 |
| docs/contracts/target-v1-closure-pack/T-03-speech-summary.md | T-03 — Privacy-safe order summary và whitelist biến lời thoại | 1/8 | 3 | 4 | 3 | 1042 |
| docs/contracts/target-v1-closure-pack/T-04-dial-token.md | T-04 — Dial-token: issue / resolve / TTL / one-use / audit | 1/8 | 4 | 2 | 3 | 978 |
| docs/contracts/target-v1-closure-pack/T-05-callback-ack.md | T-05 — Generic callback target, ACK taxonomy, idempotency/version | 1/8 | 1 | 0 | 4 | 938 |
| docs/contracts/target-v1-closure-pack/T-06-no-answer-timeout.md | T-06 — No-answer, wait-for-timeout và race lúc revalidation | 1/8 | 1 | 3 | 3 | 890 |
| docs/contracts/target-v1-closure-pack/T-07-production-auth.md | T-07 — Production service auth: JWT issuer/audience/scope/TTL/JWKS + mTLS | 1/8 | 5 | 1 | 6 | 966 |
| docs/contracts/target-v1-closure-pack/T-08-openapi-compat-cdc.md | T-08 — OpenAPI compatibility, deprecation và sở hữu consumer-driven test | 1/8 | 3 | 5 | 3 | 886 |
| docs/contracts/target-v1-closure-pack/T-09-attempt-policy.md | T-09 — `attempt_policy_version` cho production | 1/8 | 2 | 3 | 2 | 804 |
| docs/contracts/telephony-procurement-pack/lab-acceptance-report-template.md | Biểu mẫu — Báo cáo nghiệm thu lab 1 SIM | 1/10 | 5 | 2 | 0 | 1252 |
| docs/contracts/telephony-procurement-pack/R-01-vendor-requirements.md | R-01 — Yêu cầu nhà cung cấp telephony | 1/14 | 13 | 4 | 2 | 2363 |
| docs/contracts/telephony-procurement-pack/R-02-lab-package.md | R-02 — Gói lab 1 SIM thật | 1/11 | 3 | 5 | 0 | 1612 |
| docs/contracts/telephony-procurement-pack/R-03-esim32-package.md | R-03 — Gói production nhiều kênh eSIM | 1/11 | 4 | 4 | 0 | 1687 |
| docs/contracts/telephony-procurement-pack/R-04-scorecard-and-gaps.md | R-04 — Scorecard nhà cung cấp, gap register và điều khoản hợp đồng | 1/6 | 2 | 9 | 0 | 1403 |
| docs/contracts/telephony-procurement-pack/R-05-tts-audio-capability.md | R-05 — Năng lực TTS và nguồn audio | 1/7 | 7 | 5 | 0 | 1441 |
| docs/contracts/telephony-procurement-pack/README.md | Telephony Procurement Pack — 1 SIM lab và 32 eSIM production | 1/7 | 0 | 17 | 0 | 863 |
| docs/cost-model.md | Cost model — `W-0054` · `P10-3` | 1/6 | 0 | 0 | 0 | 729 |
| docs/data-governance.md | Data governance — `W-0053` · `P10-2` | 1/9 | 0 | 0 | 0 | 1465 |
| docs/documents/0. appendices/01-OPERATIONAL-FORMS.md | OPERATIONAL FORMS | 1/16 | 1 | 0 | 0 | 3384 |
| docs/documents/0. appendices/02-AUTO-GENERATED-FORM-RULES.md | AUTO GENERATED FORM RULES | 1/59 | 1 | 0 | 0 | 1635 |
| docs/documents/0. appendices/03-PRINTING-CODE-RULES.md | PRINTING CODE RULES | 1/40 | 1 | 0 | 0 | 906 |
| docs/documents/0. appendices/04-MISA-MAPPING-RULES.md | MISA MAPPING RULES | 1/40 | 1 | 0 | 0 | 1033 |
| docs/documents/0. appendices/05-MATERIAL-PACKAGING-TAXONOMY.md | MATERIAL PACKAGING TAXONOMY | 1/671 | 1 | 0 | 0 | 45469 |
| docs/documents/00-AI-EVALUATION-DEV-READINESS.md | AI Evaluation - Dev Implementation Readiness | 1/6 | 1 | 0 | 0 | 457 |
| docs/documents/00-DOC-READING-ORDER.md | Documentation Reading Order | 1/179 | 0 | 177 | 0 | 4269 |
| docs/documents/1. master/01-MASTER-00-INDEX-REGISTRY.md | MASTER-00 - INDEX REGISTRY | 1/265 | 1 | 0 | 0 | 21923 |
| docs/documents/1. master/02-MASTER-01-SOURCE-OF-TRUTH.md | MASTER-01 - SOURCE OF TRUTH | 1/253 | 1 | 0 | 0 | 17031 |
| docs/documents/1. master/03-MASTER-02-CROSS-PACK-DEPENDENCY.md | MASTER-02 - CROSS PACK DEPENDENCY | 1/482 | 1 | 0 | 0 | 23087 |
| docs/documents/1. master/04-MASTER-03-TRACEABILITY-ID.md | MASTER-03 - TRACEABILITY ID | 1/380 | 1 | 0 | 0 | 21646 |
| docs/documents/1. master/05-MASTER-04-RUNTIME-RESOLUTION-GUARD.md | MASTER-04 - RUNTIME RESOLUTION GUARD | 1/413 | 1 | 0 | 0 | 23301 |
| docs/documents/1. master/06-MASTER-05-EVIDENCE-SMOKE-COMPLETION-GATE.md | MASTER-05 - EVIDENCE SMOKE COMPLETION GATE | 1/394 | 1 | 0 | 0 | 18506 |
| docs/documents/1. master/07-MASTER-06-RESERVED-FUTURE-INTEGRATION.md | MASTER-06 - RESERVED FUTURE INTEGRATION | 1/410 | 1 | 0 | 0 | 18526 |
| docs/documents/1. master/08-MASTER-07-RELEASE-GO-LIVE-CONTROL.md | MASTER-07 - RELEASE GO LIVE CONTROL | 1/378 | 1 | 0 | 0 | 17407 |
| docs/documents/1. master/09-MASTER-08-CROSS-SYSTEM-DECISION-LOG.md | MASTER-08 - CROSS-SYSTEM DECISION LOG | 1/14 | 1 | 0 | 0 | 2317 |
| docs/documents/1. master/10-MASTER-09-CROSS-PHASE-RUNTIME-LOCK-ADDENDUM.md | MASTER-09 - CUSTOMER / PRODUCT / COMMERCE / AI / FACEBOOK / CRM / FINANCE CROSS-PHASE RUNTIME LOCK ADDENDUM | 1/20 | 1 | 0 | 0 | 1683 |
| docs/documents/2. pack/01-PACK-01-OPERATIONAL-CORE.md | PACK-01 - OPERATIONAL CORE | 1/461 | 1 | 0 | 0 | 27529 |
| docs/documents/2. pack/02-PACK-02-PRODUCT-MASTER-SKU-RECIPE-ACTIVATION.md | PACK-02 - PRODUCT MASTER SKU RECIPE ACTIVATION | 1/310 | 1 | 0 | 0 | 23982 |
| docs/documents/2. pack/03-PACK-03-DEMAND-MRP-PROCUREMENT-MATERIAL-CONTROL.md | PACK-03 - DEMAND MRP PROCUREMENT MATERIAL CONTROL | 1/315 | 1 | 0 | 0 | 20715 |
| docs/documents/2. pack/04-PACK-04-MISA-ACCOUNTING-HANDOFF.md | PACK-04 - MISA ACCOUNTING HANDOFF | 1/428 | 1 | 0 | 0 | 29288 |
| docs/documents/2. pack/05-PACK-05-AI-ADVISOR-CHANNEL.md | PACK-05 - AI ADVISOR CHANNEL | 1/354 | 1 | 0 | 0 | 31341 |
| docs/documents/2. pack/06-PACK-06-FACEBOOK-CHANNEL-GATEWAY.md | PACK-06 - FACEBOOK CHANNEL GATEWAY | 1/2048 | 1 | 0 | 0 | 29274 |
| docs/documents/2. pack/07-PACK-07-ADS-ROAS-ATTRIBUTION.md | PACK-07 - ADS ROAS ATTRIBUTION | 1/714 | 1 | 0 | 0 | 48479 |
| docs/documents/2. pack/08-PACK-08-MC-AI-LIVE.md | PACK-08 - MC AI LIVE | 1/223 | 1 | 0 | 0 | 7604 |
| docs/documents/2. pack/09-PACK-09-IVR-ORDER-CONFIRMATION.md | PACK-09 - IVR ORDER CONFIRMATION | 1/475 | 1 | 0 | 0 | 25646 |
| docs/documents/2. pack/10-PACK-10-COMPLETION-EVIDENCE-GATEWAY.md | PACK-10 - COMPLETION EVIDENCE GATEWAY | 1/308 | 1 | 0 | 0 | 20005 |
| docs/documents/3. tech/01-TECH-00-GINSENGFOOD-TECHNICAL-IMPLEMENTATION-MASTER-PLAN.md | TECH-00 - GINSENGFOOD TECHNICAL IMPLEMENTATION MASTER PLAN | 1/578 | 1 | 0 | 0 | 14333 |
| docs/documents/3. tech/02-TECH-01-FOUNDATION-RBAC-AUDIT-IDEMPOTENCY-EVIDENCE-REGISTRY.md | TECH-01 - FOUNDATION RBAC AUDIT IDEMPOTENCY EVIDENCE REGISTRY | 1/618 | 1 | 0 | 0 | 13377 |
| docs/documents/3. tech/03-TECH-02-PRODUCT-SKU-INGREDIENT-RECIPE-FORMULA-PRODUCT-ACTIVATION.md | TECH-02 - PRODUCT SKU INGREDIENT RECIPE FORMULA PRODUCT ACTIVATION | 1/719 | 1 | 0 | 0 | 15842 |
| docs/documents/3. tech/04-TECH-03-OPERATIONAL-CORE-PRODUCTION-WAREHOUSE-INVENTORY-TRACEABILITY-RECALL-SALE-LOCK.md | TECH-03 - OPERATIONAL CORE PRODUCTION WAREHOUSE INVENTORY TRACEABILITY RECALL SALE LOCK | 1/1657 | 1 | 0 | 0 | 27489 |
| docs/documents/3. tech/05-TECH-04-COMMERCE-RUNTIME-SELLABLE-GATE-QUOTE-CART-ORDER-PAYMENT-SHIPPING.md | TECH-04 - COMMERCE RUNTIME SELLABLE GATE QUOTE CART ORDER PAYMENT SHIPPING | 1/1996 | 1 | 0 | 0 | 31132 |
| docs/documents/3. tech/06-TECH-05-AI-ADVISOR-RUNTIME-PRODUCT-CONSULTING-CUSTOMER-MEMORY-QUOTE-SAFE-SALES-ORDER-DRAFT-HANDOFF.md | TECH-05 - AI ADVISOR RUNTIME PRODUCT CONSULTING CUSTOMER MEMORY QUOTE SAFE SALES ORDER DRAFT HANDOFF | 1/1904 | 1 | 0 | 0 | 31916 |
| docs/documents/3. tech/07-TECH-06-FACEBOOK-GATEWAY-META-LIVE-MESSENGER-CHANNEL-DELIVERY-COMMENT-TO-MESSENGER.md | TECH-06 - FACEBOOK GATEWAY META LIVE MESSENGER CHANNEL DELIVERY COMMENT TO MESSENGER | 1/1863 | 1 | 0 | 0 | 29255 |
| docs/documents/3. tech/08-TECH-07-ADS-MEASUREMENT-ATTRIBUTION-ROAS-CPA-AOV-VERIFIED-REVENUE-SCALE-GATE.md | TECH-07 - ADS MEASUREMENT ATTRIBUTION ROAS CPA AOV VERIFIED REVENUE SCALE GATE | 1/959 | 1 | 0 | 0 | 28841 |
| docs/documents/3. tech/09-TECH-08-MC-AI-LIVE-LIVE-SCRIPT-RUNTIME-HOSTING-INTELLIGENCE-LIVE-SALES-CONTROL-ADS-SAFE-LIVE-ORCHESTRATION.md | TECH-08 - MC AI LIVE LIVE SCRIPT RUNTIME HOSTING INTELLIGENCE LIVE SALES CONTROL ADS SAFE LIVE ORCHESTRATION | 1/927 | 1 | 0 | 0 | 30519 |
| docs/documents/3. tech/10-TECH-09-IVR-ORDER-CONFIRMATION-AUTO-CALL-VERIFICATION-ANTI-FAKE-ORDER-CONTROL.md | TECH-09 - IVR ORDER CONFIRMATION AUTO CALL VERIFICATION ANTI FAKE ORDER CONTROL | 1/650 | 1 | 0 | 0 | 25871 |
| docs/documents/3. tech/11-TECH-10-GLOBAL-SMOKE-UAT-EVIDENCE-RELEASE-GATEWAY-PRODUCTION-READINESS-CONTROL.md | TECH-10 - GLOBAL SMOKE UAT EVIDENCE RELEASE GATEWAY PRODUCTION READINESS CONTROL | 1/768 | 1 | 0 | 0 | 21782 |
| docs/documents/3. tech/12-TECH-11-IMPLEMENTATION-ROADMAP-DEV-PHASE-PLAN-BACKLOG-GOVERNANCE-CODE-HANDOFF-CONTROL.md | TECH-11 - IMPLEMENTATION ROADMAP DEV PHASE PLAN BACKLOG GOVERNANCE CODE HANDOFF CONTROL | 1/704 | 1 | 0 | 0 | 20236 |
| docs/documents/3. tech/13-TECH-12-PHASE-BACKLOG-PACK-DEV-TASK-BREAKDOWN-SOURCE-TO-BACKLOG-MATRIX.md | TECH-12 - PHASE BACKLOG PACK DEV TASK BREAKDOWN SOURCE TO BACKLOG MATRIX | 1/1055 | 1 | 0 | 0 | 22127 |
| docs/documents/3. tech/14-TECH-13-CODEX-COPILOT-DEV-PROMPT-PACK-PHASE-EXECUTION-HANDOFF-IMPLEMENTATION-REPORT-TEMPLATE.md | TECH-13 - CODEX COPILOT DEV PROMPT PACK PHASE EXECUTION HANDOFF IMPLEMENTATION REPORT TEMPLATE | 1/739 | 1 | 0 | 0 | 20608 |
| docs/documents/4. phase/phase-1/00-PHÂN TÍCH NGUỒN SỰ THẬT VÀ PHẠM VI.md | 00-P1 - PHÂN TÍCH NGUỒN SỰ THẬT VÀ PHẠM VI | 2/96 | 1 | 0 | 0 | 5411 |
| docs/documents/4. phase/phase-1/01-THIẾT KẾ KỸ THUẬT SẢN PHẨM SKU CÔNG THỨC.md | 01-P1 - THIẾT KẾ KỸ THUẬT SẢN PHẨM SKU CÔNG THỨC | 2/147 | 1 | 0 | 0 | 5407 |
| docs/documents/4. phase/phase-1/02-SẢN PHẨM SKU NGUYÊN LIỆU VÀ ĐƠN VỊ TÍNH.md | 02-P1 - SẢN PHẨM SKU NGUYÊN LIỆU ĐƠN VỊ TÍNH | 9/186 | 1 | 0 | 0 | 35006 |
| docs/documents/4. phase/phase-1/03-CÔNG THỨC BOM PHIÊN BẢN G1.md | 03-P1 - CÔNG THỨC BOM PHIÊN BẢN G1 | 18/325 | 1 | 0 | 0 | 48771 |
| docs/documents/4. phase/phase-1/04-KHÓA KÍCH HOẠT SẢN PHẨM.md | 04-P1 - KHÓA KÍCH HOẠT SẢN PHẨM | 2/51 | 1 | 0 | 0 | 2002 |
| docs/documents/4. phase/phase-1/05-QUẢN TRỊ DỮ LIỆU KHỞI TẠO MỞ RỘNG SKU VÀ VẬT TƯ.md | 05-P1 - QUẢN TRỊ SEED MỞ RỘNG SKU VÀ VẬT TƯ | 9/166 | 1 | 0 | 0 | 13998 |
| docs/documents/4. phase/phase-1/06-KIỂM THỬ KHÓI BẰNG CHỨNG VÀ NGHIỆM THU.md | 06-P1 - SMOKE BẰNG CHỨNG VÀ NGHIỆM THU | 2/643 | 1 | 0 | 0 | 12706 |
| docs/documents/4. phase/phase-1/07-BẢNG PHIẾU LỆNH SẢN XUẤT.md | 07 BẢNG PHIẾU LỆNH SẢN XUẤT | 0/0 | 1 | 0 | 0 | 3599 |
| docs/documents/4. phase/phase-1/08-BỘ KHÓA 5 BƯỚC.md | BỘ ĐẶC TẢ MÀN HÌNH / ROUTE / ACTION CHO TOÀN BỘ PHIẾU VẬN HÀNH - GINSENGFOOD | 16/148 | 1 | 0 | 0 | 23025 |
| docs/documents/4. phase/phase-1/09-CÔNG THỨC VẬN HÀNH 20 SKU MỚI.md | 09 CÔNG THỨC VẬN HÀNH 20 SKU MỚI | 0/0 | 1 | 0 | 0 | 18505 |
| docs/documents/4. phase/phase-1/10-DANH MỤC PHÂN NHÓM NGUYÊN LIỆU VÀ VẬT TƯ.md | 1\. Nguyên tắc phân nhóm tổng thể | 7/22 | 1 | 0 | 0 | 9190 |
| docs/documents/4. phase/phase-1/11-PHIẾU TỰ SINH IN KẾ TOÁN VÀ HOẠCH TOÁN.md | 1\. QUY TẮC TOÀN BỘ PHIẾU TỰ SINH - GINSENGFOOD | 1/1 | 1 | 0 | 0 | 3858 |
| docs/documents/4. phase/phase-2/00-PHÂN TÍCH VẬN HÀNH LÕI.md | 00-P2 - PHÂN TÍCH VẬN HÀNH LÕI | 2/167 | 1 | 0 | 0 | 6524 |
| docs/documents/4. phase/phase-2/01-THIẾT KẾ KỸ THUẬT VẬN HÀNH LÕI.md | 01-P2 - THIẾT KẾ KỸ THUẬT VẬN HÀNH LÕI | 3/282 | 1 | 0 | 0 | 10086 |
| docs/documents/4. phase/phase-2/02-NGUỒN NGUYÊN LIỆU LÔ QC SẴN SÀNG SẢN XUẤT.md | 02-P2 - NGUỒN NGUYÊN LIỆU LÔ QC SẴN SÀNG SẢN XUẤT | 3/883 | 1 | 0 | 0 | 52170 |
| docs/documents/4. phase/phase-2/03-LỆNH SẢN XUẤT HỒ SƠ GỐC VÀ ẢNH CHỤP.md | 03-P2 - LỆNH SẢN XUẤT HỒ SƠ GỐC VÀ SNAPSHOT | 3/219 | 1 | 0 | 0 | 29343 |
| docs/documents/4. phase/phase-2/04-CẤP PHÁT NGUYÊN LIỆU NHẬN XƯỞNG VÀ SỔ KHO.md | 04-P2 - CẤP PHÁT NGUYÊN LIỆU NHẬN XƯỞNG VÀ SỔ KHO | 3/226 | 1 | 0 | 0 | 8489 |
| docs/documents/4. phase/phase-2/05-MẺ SẢN XUẤT QC PHÁT HÀNH KHO VÀ TỒN.md | 05-P2 - MẺ SẢN XUẤT QC RELEASE KHO VÀ TỒN | 3/273 | 1 | 0 | 0 | 8848 |
| docs/documents/4. phase/phase-2/06-TRUY XUẤT QR THU HỒI VÀ KHÓA BÁN.md | 06-P2 - TRUY XUẤT QR THU HỒI VÀ KHÓA BÁN | 19/445 | 1 | 0 | 0 | 35431 |
| docs/documents/4. phase/phase-2/07-KIỂM THỬ KHÓI BẰNG CHỨNG VÀ NGHIỆM THU.md | 07-P2 - SMOKE BẰNG CHỨNG VÀ NGHIỆM THU | 2/716 | 1 | 0 | 0 | 14116 |
| docs/documents/4. phase/phase-3/00-PHÂN TÍCH HIỆN TRẠNG.md | P3 - ANALYSIS ONLY | 1/362 | 1 | 0 | 0 | 7136 |
| docs/documents/4. phase/phase-3/01-THIẾT KẾ KỸ THUẬT.md | P3.1 - TECHNICAL DESIGN ONLY | 1/279 | 1 | 0 | 0 | 6792 |
| docs/documents/4. phase/phase-3/02-CỔNG ĐỦ ĐIỀU KIỆN BÁN THỜI GIAN CHẠY.md | P3.2A - SELLABLE GATE RUNTIME | 1/172 | 1 | 0 | 0 | 4938 |
| docs/documents/4. phase/phase-3/03-GIÁ CHƯƠNG TRÌNH QUYỀN LỢI THÀNH VIÊN VÀ ẢNH CHỤP BÁO GIÁ.md | P3.2B - PRICE PROGRAM MEMBER BENEFIT QUOTE SNAPSHOT | 1/187 | 1 | 0 | 0 | 5454 |
| docs/documents/4. phase/phase-3/04-GIỎ HÀNG ĐƠN NHÁP VÀ XÁC NHẬN KHÁCH HÀNG.md | P3.2C - CART ORDER DRAFT CUSTOMER CONFIRMATION | 1/210 | 1 | 0 | 0 | 5412 |
| docs/documents/4. phase/phase-3/05-ĐƠN CHÍNH THỨC MÃ ĐƠN VÀ MÁY TRẠNG THÁI.md | P3.2D - OFFICIAL ORDER ORDER CODE STATE MACHINE | 1/181 | 1 | 0 | 0 | 5650 |
| docs/documents/4. phase/phase-3/06-THANH TOÁN GIAO HÀNG HÓA ĐƠN VÀ THUẾ.md | P3.2E - PAYMENT SHIPPING INVOICE TAX | 1/263 | 1 | 0 | 0 | 6387 |
| docs/documents/4. phase/phase-3/07-DOANH THU XÁC THỰC HOA HỒNG ROAS VÀ BÀN GIAO.md | P3.2F - VERIFIED REVENUE COMMISSION ROAS HANDOFF | 1/253 | 1 | 0 | 0 | 6661 |
| docs/documents/4. phase/phase-3/08-BÁO CÁO KIỂM THỬ KHÓI VÀ BẰNG CHỨNG.md | P3.2G - SMOKE EVIDENCE REPORT | 1/662 | 1 | 0 | 0 | 9238 |
| docs/documents/4. phase/phase-3/09-CHỈ MỤC BÀN GIAO GIAI ĐOẠN 3.md | P3 - README HANDOFF INDEX | 1/85 | 1 | 0 | 0 | 3536 |
| docs/documents/4. phase/phase-3/10-PHỤ LỤC KHÓA THỜI GIAN CHẠY TỪ KHÁCH HÀNG ĐẾN THU TIỀN.md | P3 - Addendum Customer-to-Cash Runtime Lock | 1/10 | 1 | 0 | 0 | 1041 |
| docs/documents/4. phase/phase-3/11-GÓI CHUẨN DUYỆT SRS VÀ TRIỂN KHAI GIAI ĐOẠN 3.md | 11 - GÓI CHUẨN DUYỆT / SRS / TRIỂN KHAI GIAI ĐOẠN 3 | 1/22 | 1 | 0 | 0 | 2990 |
| docs/documents/4. phase/phase-3.1/00-PHÂN TÍCH HIỆN TRẠNG GIAI ĐOẠN 3.1.md | 00 - PHÂN TÍCH HIỆN TRẠNG GIAI ĐOẠN 3.1 | 1/17 | 1 | 0 | 0 | 1041 |
| docs/documents/4. phase/phase-3.1/01-THIẾT KẾ KỸ THUẬT GIAI ĐOẠN 3.1.md | 01 - THIẾT KẾ KỸ THUẬT GIAI ĐOẠN 3.1 | 1/15 | 1 | 0 | 0 | 1254 |
| docs/documents/4. phase/phase-3.1/02-THÀNH VIÊN GIÁ CHƯƠNG TRÌNH 24-7 VÀ GIỜ VÀNG.md | 02 - THÀNH VIÊN / GIÁ / CHƯƠNG TRÌNH 24/7 / GIỜ VÀNG | 1/34 | 1 | 0 | 0 | 1938 |
| docs/documents/4. phase/phase-3.1/03-DIAMOND GIỚI THIỆU HOA HỒNG VÀ ĐỐI SOÁT.md | 03 - DIAMOND / GIỚI THIỆU / HOA HỒNG / ĐỐI SOÁT | 1/24 | 1 | 0 | 0 | 1364 |
| docs/documents/4. phase/phase-3.1/04-TIN NHẮN GIỜ VÀNG CRM VÀ ĐA KÊNH.md | 04 - TIN NHẮN GIỜ VÀNG / CRM / ĐA KÊNH | 1/28 | 1 | 0 | 0 | 1465 |
| docs/documents/4. phase/phase-3.1/05-CRM 12 THÁNG VÀ VÒNG ĐỜI THÀNH VIÊN.md | 05 - CRM 12 THÁNG / VÒNG ĐỜI THÀNH VIÊN | 1/22 | 1 | 0 | 0 | 1626 |
| docs/documents/4. phase/phase-3.1/06-CỐ VẤN AI CHÍNH SÁCH LÕI VÀ TÌNH HUỐNG THỰC CHIẾN.md | 06 - AI ADVISOR / CHÍNH SÁCH LÕI / TÌNH HUỐNG THỰC CHIẾN | 1/18 | 1 | 0 | 0 | 1998 |
| docs/documents/4. phase/phase-3.1/07-THANH TOÁN VẬN CHUYỂN IVR VÀ ĐƠN HÀNG.md | 07 - THANH TOÁN / VẬN CHUYỂN / IVR / ĐƠN HÀNG | 1/30 | 1 | 0 | 0 | 1536 |
| docs/documents/4. phase/phase-3.1/08-KIỂM THỬ BẰNG CHỨNG VÀ BÁO CÁO GIAI ĐOẠN 3.1.md | 08 - KIỂM THỬ / BẰNG CHỨNG / BÁO CÁO GIAI ĐOẠN 3.1 | 1/16 | 1 | 0 | 0 | 1189 |
| docs/documents/4. phase/phase-3.1/09-HƯỚNG DẪN BÀN GIAO GIAI ĐOẠN 3.1.md | 09 - HƯỚNG DẪN BÀN GIAO GIAI ĐOẠN 3.1 | 1/14 | 1 | 0 | 0 | 840 |
| docs/documents/4. phase/phase-3.1/10-PHỤ LỤC KHÓA LIÊN GIAI ĐOẠN DIAMOND CRM VÀ AI.md | 10 - PHỤ LỤC KHÓA LIÊN GIAI ĐOẠN DIAMOND / CRM / AI | 1/20 | 1 | 0 | 0 | 1003 |
| docs/documents/4. phase/phase-3.1/11-BẢNG GOM TRIỂN KHAI DIAMOND HOA HỒNG VÀ THƯỞNG MỚI.md | BẢNG GOM NGUỒN CHÍNH GIAI ĐOẠN 3.1 - DIAMOND / THÀNH VIÊN / CRM / AI ADVISOR | 1/18 | 1 | 0 | 0 | 2169 |
| docs/documents/4. phase/phase-3.1/5. bo sung/00-BẢN TỔNG HỢP SAU KIỂM THỬ THỰC CHIẾN CHÍNH SÁCH LÕI TIN NHẮN CRM VÀ VÒNG ĐỜI THÀNH VIÊN.md | 00 BẢN TỔNG HỢP SAU KIỂM THỬ THỰC CHIẾN CHÍNH SÁCH LÕI TIN NHẮN CRM VÀ VÒNG ĐỜI THÀNH VIÊN | 0/0 | 1 | 0 | 0 | 6741 |
| docs/documents/4. phase/phase-3.1/5. bo sung/01-BẢN KHÓA BỔ SUNG TÌNH HUỐNG ĐỜI THƯỜNG TRƯỚC KHI VIẾT LẠI TÀI LIỆU.md | 01 BẢN KHÓA BỔ SUNG TÌNH HUỐNG ĐỜI THƯỜNG TRƯỚC KHI VIẾT LẠI TÀI LIỆU | 0/0 | 1 | 0 | 0 | 7107 |
| docs/documents/4. phase/phase-3.1/5. bo sung/02-BỔ SUNG TIN NHẮN CHĂM SÓC KHÁCH HÀNG.md | HỆ THỐNG TIN NHẮN GIỜ VÀNG - SOURCE BASELINE | 7/19 | 1 | 0 | 0 | 3996 |
| docs/documents/4. phase/phase-3.1/5. bo sung/03-CHÍNH SÁCH VÀ HOA HỒNG THÀNH VIÊN.md | 03 CHÍNH SÁCH VÀ HOA HỒNG THÀNH VIÊN | 0/0 | 1 | 0 | 0 | 3872 |
| docs/documents/4. phase/phase-3.1/5. bo sung/04-KHÓA TỰ ĐỘNG HÓA CRM 12 THÁNG VÀ VÒNG ĐỜI THÀNH VIÊN.md | 04 KHÓA TỰ ĐỘNG HÓA CRM 12 THÁNG VÀ VÒNG ĐỜI THÀNH VIÊN | 0/0 | 1 | 0 | 0 | 2159 |
| docs/documents/4. phase/phase-3.1/5. bo sung/05-PHỤ LỤC HỆ THỐNG TIN NHẮN.md | PHỤ LỤC HỆ THỐNG TIN NHẮN GIỜ VÀNG - CANONICAL APPENDIX | 10/29 | 1 | 0 | 0 | 4297 |
| docs/documents/4. phase/phase-4/00-PHÂN TÍCH HIỆN TRẠNG.md | P4 - PHÂN TÍCH HIỆN TRẠNG | 2/17 | 1 | 0 | 0 | 1607 |
| docs/documents/4. phase/phase-4/01-THIẾT KẾ KỸ THUẬT.md | P4.1 - TECHNICAL DESIGN ONLY | 1/76 | 1 | 0 | 0 | 8379 |
| docs/documents/4. phase/phase-4/02-HỢP ĐỒNG TIÊU THỤ DỮ LIỆU THỜI GIAN CHẠY.md | P4.2A - RUNTIME CONSUMER CONTRACT | 1/54 | 1 | 0 | 0 | 6918 |
| docs/documents/4. phase/phase-4/03-BỘ NHỚ KHÁCH HÀNG VÀ BỘ GIẢI NGỮ CẢNH.md | P4.2B - CUSTOMER MEMORY CONTEXT RESOLVER | 1/38 | 1 | 0 | 0 | 5239 |
| docs/documents/4. phase/phase-4/04-ĐIỀU PHỐI TƯ VẤN KIẾN THỨC SẢN PHẨM.md | P4.2C - PRODUCT KNOWLEDGE CONSULTATION ORCHESTRATOR | 1/55 | 1 | 0 | 0 | 6835 |
| docs/documents/4. phase/phase-4/05-TIÊU THỤ BÁO GIÁ NHÁP VÀ XÁC NHẬN ĐƠN.md | P4.2D - QUOTE ORDER DRAFT CONFIRMATION CONSUMER | 1/61 | 1 | 0 | 0 | 6213 |
| docs/documents/4. phase/phase-4/06-CHẶN AN TOÀN NGÔN TỪ CÔNG KHAI VÀ TUYÊN BỐ.md | P4.2E - SAFETY PUBLIC WORDING CLAIM GUARD | 1/68 | 1 | 0 | 0 | 5447 |
| docs/documents/4. phase/phase-4/07-CẦU NỐI BÀN GIAO KÊNH MESSENGER TRỰC TIẾP VÀ CRM.md | P4.2F - CHANNEL HANDOFF MESSENGER LIVE CRM BRIDGE | 1/47 | 1 | 0 | 0 | 5120 |
| docs/documents/4. phase/phase-4/08-BÁO CÁO KIỂM THỬ KHÓI VÀ BẰNG CHỨNG.md | P4.2G - SMOKE EVIDENCE REPORT | 1/41 | 1 | 0 | 0 | 4199 |
| docs/documents/4. phase/phase-4/09-CHỈ MỤC BÀN GIAO.md | P4 - CHỈ MỤC BÀN GIAO | 1/8 | 1 | 0 | 0 | 1325 |
| docs/documents/4. phase/phase-4/10-PHỤ LỤC KHÓA THỜI GIAN CHẠY THỰC CHIẾN CỐ VẤN AI.md | P4 - PHỤ LỤC KHÓA RUNTIME THỰC CHIẾN AI ADVISOR | 1/11 | 1 | 0 | 0 | 909 |
| docs/documents/4. phase/phase-4/11-MA TRẬN SRS VÀ BÀN GIAO TRIỂN KHAI.md | P4 - MA TRẬN SRS VÀ BÀN GIAO TRIỂN KHAI | 1/14 | 1 | 0 | 0 | 2624 |
| docs/documents/4. phase/phase-4/12-BẢNG GOM GIAI ĐOẠN 4.md | BẢNG GÔM GIAI ĐOẠN 4 - AI ADVISOR RUNTIME CANONICAL REWRITE | 1/52 | 1 | 0 | 0 | 5760 |
| docs/documents/4. phase/phase-5/00-PHÂN TÍCH HIỆN TRẠNG.md | P5 - Analysis Only | 1/17 | 1 | 0 | 0 | 1660 |
| docs/documents/4. phase/phase-5/01-THIẾT KẾ KỸ THUẬT.md | P5.1 - Technical Design Only | 1/21 | 1 | 0 | 0 | 1799 |
| docs/documents/4. phase/phase-5/02-ĐỊNH DANH KÊNH VÀ NGỮ CẢNH TRANG.md | P5.2A - Channel Identity / Page Context | 1/16 | 1 | 0 | 0 | 1166 |
| docs/documents/4. phase/phase-5/03-BÌNH LUẬN CÔNG KHAI VÀ BÀN GIAO MESSENGER.md | P5.2B - Public Comment Boundary / Comment-to-Messenger | 1/17 | 1 | 0 | 0 | 792 |
| docs/documents/4. phase/phase-5/04-NGỮ CẢNH MESSENGER VÀ CHẶN PHẢN HỒI CUỐI.md | P5.2C - Messenger Context / Final Response Guard | 1/17 | 1 | 0 | 0 | 666 |
| docs/documents/4. phase/phase-5/05-CHỈ BÁO ĐANG GÕ VÀ NHỊP ĐỘ PHẢN HỒI.md | P5.2D - Typing Indicator / Response Pacing | 1/17 | 1 | 0 | 0 | 620 |
| docs/documents/4. phase/phase-5/06-GIỚI HẠN TẦN SUẤT CHỐNG SPAM VÀ KIỂM DUYỆT.md | P5.2E - Rate Limit / Anti-Spam / Moderation | 1/17 | 1 | 0 | 0 | 934 |
| docs/documents/4. phase/phase-5/07-CHẶN GỬI TỪ CHỐI NHẬN VÀ BÀN GIAO NGƯỜI THẬT.md | P5.2F - Suppression / Opt-out / Human Handoff | 1/17 | 1 | 0 | 0 | 930 |
| docs/documents/4. phase/phase-5/08-BÁO CÁO KIỂM THỬ KHÓI VÀ BẰNG CHỨNG CỔNG FACEBOOK.md | P5.2G - Gateway Smoke / Evidence Report | 1/20 | 1 | 0 | 0 | 1392 |
| docs/documents/4. phase/phase-5/09-CHỈ MỤC BÀN GIAO.md | P5 - CHỈ MỤC BÀN GIAO | 1/15 | 1 | 0 | 0 | 1289 |
| docs/documents/4. phase/phase-5/10-PHỤ LỤC KHÓA THỜI GIAN CHẠY CỔNG FACEBOOK VÀ NHẮN TIN.md | P5 - PHỤ LỤC KHÓA RUNTIME GATEWAY FACEBOOK VÀ NHẮN TIN | 1/21 | 1 | 0 | 0 | 1265 |
| docs/documents/4. phase/phase-5/11-BẢNG GOM GIAI ĐOẠN 5.md | BẢNG GÔM GIAI ĐOẠN 5 - FACEBOOK CHANNEL GATEWAY RUNTIME LOCK | 1/38 | 1 | 0 | 0 | 3993 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/00-KHÓA NỀN TẢNG TÀI CHÍNH KINH DOANH VÀ QUẢNG CÁO.md | 00 KHÓA NỀN TẢNG TÀI CHÍNH KINH DOANH VÀ QUẢNG CÁO | 0/0 | 1 | 0 | 0 | 2632 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/01-MÔ HÌNH VẬN HÀNH QUẢNG CÁO FACEBOOK VÀ BÁN HÀNG TRỰC TIẾP.md | 01. FILE FACEBOOK ADS & LIVE COMMERCE OPERATING MODEL | 1/10 | 1 | 0 | 0 | 901 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/02-BÁO CÁO HOÀN TẤT CỐ VẤN AI FACEBOOK.md | 02 BÁO CÁO HOÀN TẤT CỐ VẤN AI FACEBOOK | 0/0 | 1 | 0 | 0 | 7659 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/03-DANH SÁCH FACEBOOK AI.md | 03 DANH SÁCH FACEBOOK AI | 0/0 | 1 | 0 | 0 | 1002 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/04-MÔ HÌNH GIÁ GIỜ VÀNG TRI ÂN DOANH THU BÁN HÀNG TRỰC TIẾP QUẢNG CÁO VÀ DIAMOND.md | 04 MÔ HÌNH GIÁ GIỜ VÀNG TRI ÂN DOANH THU BÁN HÀNG TRỰC TIẾP QUẢNG CÁO VÀ DIAMOND | 0/0 | 1 | 0 | 0 | 1673 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/05-MÔ HÌNH CHI PHÍ QUẢNG CÁO VÀ MỞ RỘNG BÁN HÀNG TRỰC TIẾP.md | 05 MÔ HÌNH CHI PHÍ QUẢNG CÁO VÀ MỞ RỘNG BÁN HÀNG TRỰC TIẾP | 0/0 | 1 | 0 | 0 | 969 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/06-KHÓA KÊNH AI ĐỦ ĐIỀU KIỆN TRƯỚC CỔNG FACEBOOK.md | 06 KHÓA KÊNH AI ĐỦ ĐIỀU KIỆN TRƯỚC CỔNG FACEBOOK | 0/0 | 1 | 0 | 0 | 11841 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/07-PHÂN TÍCH MÔ HÌNH TÀI CHÍNH.md | 07 PHÂN TÍCH MÔ HÌNH TÀI CHÍNH | 0/0 | 1 | 0 | 0 | 1012 |
| docs/documents/4. phase/phase-5/TÀI LIỆU BỔ SUNG/08-TRÌNH TỰ TRIỂN KHAI TIẾP THEO.md | BỘ FACEBOOK CHANNEL GATEWAY NÊN VIẾT TIẾP | 1/24 | 1 | 0 | 0 | 2943 |
| docs/documents/4. phase/phase-6/00-PHÂN TÍCH HIỆN TRẠNG.md | P6 - Analysis Only | 1/8 | 1 | 0 | 0 | 302 |
| docs/documents/4. phase/phase-6/01-THIẾT KẾ KỸ THUẬT.md | P6.1 - Technical Design Only | 1/9 | 1 | 0 | 0 | 341 |
| docs/documents/4. phase/phase-6/02-PHÂN LOẠI SỰ KIỆN VÀ TÍN HIỆU PHỄU BÁN HÀNG.md | P6.2A - Event Taxonomy / Funnel Signal Boundary | 1/8 | 1 | 0 | 0 | 196 |
| docs/documents/4. phase/phase-6/03-PIXEL CAPI CHỐNG TRÙNG VÀ PHÂN BỔ GHI NHẬN.md | P6.2B - Pixel / CAPI Dedup / Attribution Resolver | 1/8 | 1 | 0 | 0 | 213 |
| docs/documents/4. phase/phase-6/04-DOANH THU ĐÃ XÁC THỰC VÀ QUY TẮC LOẠI TRỪ.md | P6.2C - Verified Revenue Consumption / Exclusion Rules | 1/8 | 1 | 0 | 0 | 229 |
| docs/documents/4. phase/phase-6/05-TÍNH TOÁN CPA AOV VÀ ROAS.md | P6.2D - CPA / AOV / ROAS Computation Boundary | 1/8 | 1 | 0 | 0 | 212 |
| docs/documents/4. phase/phase-6/06-CHẤT LƯỢNG DỮ LIỆU CỔNG MỞ RỘNG VÀ CHẶN.md | P6.2E - Data Quality / Scale Gate / Suppression | 1/8 | 1 | 0 | 0 | 220 |
| docs/documents/4. phase/phase-6/07-BẢNG ĐIỀU KHIỂN BẰNG CHỨNG VÀ RÀ SOÁT CHỦ SỞ HỮU.md | P6.2F - Dashboard Evidence / Owner Review Boundary | 1/8 | 1 | 0 | 0 | 238 |
| docs/documents/4. phase/phase-6/08-BÁO CÁO KIỂM THỬ KHÓI VÀ BẰNG CHỨNG QUẢNG CÁO.md | P6.2G - Ads Smoke / Evidence Report | 1/7 | 1 | 0 | 0 | 332 |
| docs/documents/4. phase/phase-6/09-CHỈ MỤC BÀN GIAO GIAI ĐOẠN 6.md | P6 - README HANDOFF INDEX | 1/8 | 1 | 0 | 0 | 310 |
| docs/documents/4. phase/phase-6/10-BẢNG GOM GIAI ĐOẠN 6.md | Phase 6 - Consolidated Handoff Index | 9/73 | 1 | 0 | 0 | 2458 |
| docs/documents/4. phase/phase-7/00-PHÂN TÍCH HIỆN TRẠNG.md | P7 - Analysis Only | 1/17 | 1 | 0 | 0 | 1446 |
| docs/documents/4. phase/phase-7/01-THIẾT KẾ KỸ THUẬT.md | P7.1 - Technical Design Only | 1/19 | 1 | 0 | 0 | 1280 |
| docs/documents/4. phase/phase-7/02-BỘ GIẢI NGỮ CẢNH PHIÊN BÁN HÀNG TRỰC TIẾP.md | P7.2A - Live Session Context Resolver | 1/18 | 1 | 0 | 0 | 772 |
| docs/documents/4. phase/phase-7/03-KỊCH BẢN BÁN HÀNG TRỰC TIẾP VÀ CHẶN TUYÊN BỐ.md | P7.2B - Live Script Runtime / Claim Guard | 1/19 | 1 | 0 | 0 | 957 |
| docs/documents/4. phase/phase-7/04-PHÂN LOẠI BÌNH LUẬN BÁN HÀNG TRỰC TIẾP VÀ ĐIỀU PHỐI MESSENGER.md | P7.2C - Live Comment Classifier / Messenger Coordination | 1/18 | 1 | 0 | 0 | 887 |
| docs/documents/4. phase/phase-7/05-RANH GIỚI SẢN PHẨM VÀ GIÁ TRONG BÁN HÀNG TRỰC TIẾP.md | P7.2D - Product / Price Boundary In Live | 1/18 | 1 | 0 | 0 | 893 |
| docs/documents/4. phase/phase-7/06-TUYẾN KIỂM DUYỆT PHÁ RỐI LẠM DỤNG VÀ KHIẾU NẠI.md | P7.2E - Troll / Abuse / Complaint Route | 1/18 | 1 | 0 | 0 | 806 |
| docs/documents/4. phase/phase-7/07-RANH GIỚI TÍN HIỆU BÁN HÀNG TRỰC TIẾP AN TOÀN CHO QUẢNG CÁO.md | P7.2F - Ads-safe Live Signal Boundary | 1/18 | 1 | 0 | 0 | 876 |
| docs/documents/4. phase/phase-7/08-BÁO CÁO KIỂM THỬ KHÓI VÀ BẰNG CHỨNG MC AI BÁN HÀNG TRỰC TIẾP.md | P7.2G - MC AI Live Smoke / Evidence Report | 1/14 | 1 | 0 | 0 | 1150 |
| docs/documents/4. phase/phase-7/09-CHỈ MỤC BÀN GIAO GIAI ĐOẠN 7.md | P7 - README HANDOFF INDEX | 1/16 | 1 | 0 | 0 | 905 |
| docs/documents/4. phase/phase-7/10-BẢNG GOM GIAI ĐOẠN 7.md | Phase 7 - Consolidated Handoff Index | 11/89 | 1 | 0 | 0 | 2968 |
| docs/documents/4. phase/phase-8/00-QUẢN TRỊ NGUỒN SỰ THẬT VÀ PHẠM VI.md | IVR-00 - Governance / Source of Truth / Scope Boundary | 1/18 | 1 | 0 | 0 | 1895 |
| docs/documents/4. phase/phase-8/01-MỤC ĐÍCH KINH DOANH VÀ CA SỬ DỤNG XÁC NHẬN.md | IVR-01 - Business Purpose / Confirmation Use Case | 1/22 | 1 | 0 | 0 | 1930 |
| docs/documents/4. phase/phase-8/02-RANH GIỚI SỞ HỮU VÀ HỆ THỐNG KẾT NỐI.md | IVR-02 - Ownership Boundary / Connected Systems | 1/17 | 1 | 0 | 0 | 1604 |
| docs/documents/4. phase/phase-8/03-ĐIỀU KIỆN GỌI NIỀM TIN KHÁCH HÀNG VÀ LIÊN HỆ CHÍNH THỨC.md | IVR-03 - Eligibility Rule / Customer Trust / Official Contact | 1/19 | 1 | 0 | 0 | 1762 |
| docs/documents/4. phase/phase-8/04-HỢP ĐỒNG TỪ LÕI ĐƠN HÀNG ĐẾN TÁC VỤ IVR.md | IVR-04 - Order Core -> IVR Task Contract | 1/17 | 1 | 0 | 0 | 1642 |
| docs/documents/4. phase/phase-8/05-CHÍNH SÁCH GỌI LẠI BỘ LẬP LỊCH VÀ HÀNG ĐỢI.md | IVR-05 - Attempt Policy / Scheduler / Queue | 1/18 | 1 | 0 | 0 | 1630 |
| docs/documents/4. phase/phase-8/06-BỘ CHUYỂN ĐỔI CỔNG SIM NỘI BỘ.md | IVR-06 - Internal SIM Gateway Adapter | 1/18 | 1 | 0 | 0 | 1204 |
| docs/documents/4. phase/phase-8/07-CHUẨN HÓA KẾT QUẢ VÀ CALLBACK VỀ LÕI ĐƠN HÀNG.md | IVR-07 - Result Normalization / IVR -> Order Core Callback | 1/17 | 1 | 0 | 0 | 1628 |
| docs/documents/4. phase/phase-8/08-GIÁM SÁT QUẢN TRỊ BẰNG CHỨNG KIỂM TOÁN VÀ RIÊNG TƯ.md | IVR-08 - Admin Monitoring / Evidence / Audit / Privacy | 1/18 | 1 | 0 | 0 | 1772 |
| docs/documents/4. phase/phase-8/09-MA TRẬN KIỂM THỬ KHÓI VÀ CỔNG PHÁT HÀNH.md | IVR-09 - Test Matrix / Smoke / Release Gate | 1/17 | 1 | 0 | 0 | 1707 |
| docs/documents/4. phase/phase-8/10-KIẾN TRÚC TRIỂN KHAI.md | IVR-10 - Implementation Architecture | 1/11 | 2 | 0 | 0 | 1185 |
| docs/documents/4. phase/phase-8/11-THIẾT KẾ API.md | IVR-11 - API Design | 1/12 | 1 | 0 | 0 | 1122 |
| docs/documents/4. phase/phase-8/12-THIẾT KẾ CƠ SỞ DỮ LIỆU.md | IVR-12 - Database Design | 1/14 | 1 | 0 | 0 | 2159 |
| docs/documents/4. phase/phase-8/13-THIẾT KẾ HÀM VÀ DỊCH VỤ.md | IVR-13 - Function / Service Design | 1/14 | 1 | 0 | 0 | 1256 |
| docs/documents/4. phase/phase-8/14-ĐIỀU PHỐI QUY TRÌNH.md | IVR-14 - Workflow Orchestration | 1/14 | 1 | 0 | 0 | 1133 |
| docs/documents/4. phase/phase-8/15-BẢO MẬT RIÊNG TƯ VÀ KIỂM TOÁN.md | IVR-15 - Security / Privacy / Audit | 1/13 | 1 | 0 | 0 | 1087 |
| docs/documents/4. phase/phase-8/16-YÊU CẦU PHI CHỨC NĂNG.md | IVR-16 - Non-Functional Requirements | 1/14 | 2 | 0 | 0 | 925 |
| docs/documents/4. phase/phase-8/17-THIẾT KẾ TÍCH HỢP.md | IVR-17 - Integration Design | 1/14 | 1 | 0 | 0 | 1059 |
| docs/documents/4. phase/phase-8/18-TRIỂN KHAI QUAN SÁT VÀ SỔ TAY VẬN HÀNH.md | IVR-18 - Deployment / Observability / Runbook | 1/13 | 1 | 0 | 0 | 1012 |
| docs/documents/4. phase/phase-8/19-KẾ HOẠCH KIỂM THỬ KHÓI VÀ PHÁT HÀNH.md | IVR-19 - Test / Smoke / Release Plan | 1/13 | 1 | 0 | 0 | 1097 |
| docs/documents/4. phase/phase-8/20-KẾ HOẠCH GIAI ĐOẠN TÁC VỤ VÀ VIỆC CẦN LÀM.md | IVR-20 - Phase / Task / TODO Plan | 1/22 | 1 | 0 | 0 | 1889 |
| docs/documents/4. phase/phase-8/21-QUYẾT ĐỊNH DỌN DẸP CHÍNH SÁCH GỌI LẠI.md | IVR-21 - Attempt Policy Cleanup Decision | 1/6 | 1 | 0 | 0 | 474 |
| docs/documents/4. phase/phase-8/22-ĐƯỜNG CƠ SỞ ĐẦU VÀO IVR.md | 22 ĐƯỜNG CƠ SỞ ĐẦU VÀO IVR | 0/0 | 1 | 0 | 0 | 2790 |
| docs/documents/4. phase/phase-8/23-XÁC NHẬN ĐƠN HÀNG BẰNG IVR.md | 23 XÁC NHẬN ĐƠN HÀNG BẰNG IVR | 0/0 | 1 | 0 | 0 | 2568 |
| docs/documents/4. phase/phase-8/24-ĐÓNG KHOẢNG TRỐNG TIỀN SRS IVR.md | IVR Phase 8 - Đặc Tả Gap Closure Tiền SRS | 1/15 | 1 | 0 | 0 | 1569 |
| docs/documents/4. phase/phase-8/25-MA TRẬN TRUY VẾT SRS IVR.md | IVR-SRS Trace Matrix - Phase 8 | 1/7 | 1 | 0 | 0 | 1149 |
| docs/documents/4. phase/phase-8/26-BẢNG GOM GIAI ĐOẠN 8.md | BẢNG GÔM PHASE 8 | 11/184 | 1 | 0 | 0 | 16789 |
| docs/documents/6. canonical/01-CANONICAL-CRM-MEMBER-LIFECYCLE-RUNTIME.md | Canonical CRM / Member Lifecycle Runtime | 1/11 | 1 | 0 | 0 | 1075 |
| docs/documents/6. canonical/02-CANONICAL-FINANCE-DIAMOND-COMMISSION-PAYOUT-RUNTIME.md | Canonical Finance / Diamond Commission / Payout Runtime | 1/10 | 1 | 0 | 0 | 775 |
| docs/documents/6. canonical/03-CANONICAL-EVIDENCE-SMOKE-GATE-CUSTOMER-TO-CASH-CARE.md | Canonical Evidence / Smoke Gate - Customer-to-Cash-to-Care | 1/7 | 1 | 0 | 0 | 721 |
| docs/documents/6. canonical/README.md | Canonical Runtime Addenda | 1/1 | 1 | 0 | 0 | 73 |
| docs/dr-topology.md | DR topology — `W-0053` · `P10-2` | 1/9 | 0 | 0 | 0 | 1381 |
| docs/evidence/W-0010/README.md | W-0010 — P0-1 Repo & Solution Bootstrap Evidence | 1/6 | 0 | 0 | 0 | 610 |
| docs/evidence/W-0011/README.md | W-0011 — P0-2 GitLab CI Baseline Evidence | 1/6 | 0 | 0 | 0 | 907 |
| docs/evidence/W-0012/README.md | W-0012 — P0-3 Cross-Cutting Foundation Evidence | 1/8 | 0 | 0 | 0 | 1081 |
| docs/evidence/W-0013/README.md | W-0013 — P0-4 Feature-Flag Platform Evidence | 1/7 | 0 | 0 | 0 | 1236 |
| docs/evidence/W-0014/README.md | W-0014 — P1-1 OpenAPI Codegen and Contract Scaffold Evidence | 1/8 | 0 | 0 | 0 | 1098 |
| docs/evidence/W-0015/README.md | W-0015 / P1-2 PostgreSQL persistence evidence | 1/7 | 0 | 0 | 0 | 740 |
| docs/evidence/W-0016/README.md | W-0016 / P1-3 — Domain, DTO mapping, provider ports and privacy guards | 1/8 | 0 | 0 | 0 | 802 |
| docs/evidence/W-0017/README.md | W-0017 / P1-4 API documentation portal evidence | 1/8 | 0 | 0 | 0 | 762 |
| docs/evidence/W-0018/README.md | W-0018 / P2-1 — Target V1 Task Intake | 1/8 | 0 | 0 | 0 | 607 |
| docs/evidence/W-0019/README.md | W-0019 / P2-2 — Eligibility and Blockers | 1/8 | 0 | 0 | 0 | 570 |
| docs/evidence/W-0020/README.md | W-0020 / P2-3 — Policy Registry, Deadline Scheduler and Channel Leases | 1/9 | 0 | 0 | 0 | 983 |
| docs/evidence/W-0021/README.md | W-0021 / P2-4 — speech rendering, dial-token and MOCK SIM adapter | 1/9 | 0 | 0 | 0 | 1026 |
| docs/evidence/W-0022/README.md | W-0022 / P2-5 — DTMF and disposition normalization | 1/7 | 0 | 0 | 0 | 1037 |
| docs/evidence/W-0023/README.md | W-0023 / P2-6 — Sales callback outbox and Golden Hour compatibility | 1/8 | 0 | 0 | 0 | 1059 |
| docs/evidence/W-0024/privacy-test-report.md | W-0024 privacy test report | 1/1 | 1 | 0 | 0 | 206 |
| docs/evidence/W-0024/README.md | W-0024 / P2-7 — Script, Content Approval and Speech Safety | 1/6 | 0 | 1 | 0 | 427 |
| docs/evidence/W-0025/README.md | W-0025 — P3-1 Admin UI Foundation | 1/11 | 0 | 0 | 0 | 1417 |
| docs/evidence/W-0026/README.md | W-0026 — P3-2 Dashboard, Call Log & Call Detail | 1/8 | 0 | 0 | 1 | 1118 |
| docs/evidence/W-0027/README.md | W-0027 — P3-3 Config, Integration Status, Review Queue, Seed/Mock & Roles | 1/9 | 0 | 0 | 0 | 1170 |
| docs/evidence/W-0028/README.md | W-0028 / P3-4 — Reporting & analytics console | 1/6 | 0 | 0 | 0 | 1080 |
| docs/evidence/W-0029/README.md | W-0029 — Evidence: Sales provider wiring and contract verification (`P4-1`) | 1/10 | 0 | 0 | 0 | 1234 |
| docs/evidence/W-0030/README.md | W-0030 — Evidence: Sales eligibility/blocker snapshot contract (`P4-2`) | 1/10 | 0 | 2 | 1 | 1502 |
| docs/evidence/W-0031/README.md | W-0031 — Evidence: Voice call restriction and trust snapshot (`P4-3`) | 1/9 | 0 | 0 | 0 | 1331 |
| docs/evidence/W-0032/README.md | W-0032 — Evidence: Service JWT, optional mTLS and audit (`P4-4`) | 1/9 | 0 | 2 | 0 | 1325 |
| docs/evidence/W-0033/README.md | W-0033 — Evidence: V1 notification disabled boundary (`P4-5`) | 1/6 | 0 | 0 | 0 | 726 |
| docs/evidence/W-0034/README.md | W-0034 — Evidence: Opt-out feedback loop (`P4-6`) | 1/9 | 0 | 0 | 0 | 1063 |
| docs/evidence/W-0035/README.md | W-0035 — Evidence: Unit & integration test suite (`P5-1`) | 1/7 | 0 | 0 | 0 | 1000 |
| docs/evidence/W-0036/README.md | W-0036 — Evidence: Contract & E2E test suite (`P5-2`) | 1/7 | 0 | 0 | 0 | 940 |
| docs/evidence/W-0037/README.md | W-0037 — Evidence: Performance, load and security/privacy tests (`P5-3`) | 1/8 | 0 | 1 | 0 | 839 |
| docs/evidence/W-0038/README.md | W-0038 — Evidence: Code review gate & static analysis (`P5-4`) | 1/7 | 0 | 0 | 0 | 971 |
| docs/evidence/W-0039/README.md | W-0039 — Evidence: Accessibility, i18n & cross-browser QA (`P5-5`) | 1/9 | 0 | 0 | 0 | 927 |
| docs/evidence/W-0040/README.md | W-0040 — Evidence: Logging, metrics & tracing (`P6-1`) | 1/18 | 0 | 0 | 0 | 2958 |
| docs/evidence/W-0041/README.md | W-0041 — Evidence: Dashboards, SLO & alerting (`P6-2`) | 1/7 | 0 | 0 | 0 | 1558 |
| docs/evidence/W-0042/README.md | W-0042 — Evidence: Chaos & resilience game-days (`P6-3`) | 1/14 | 0 | 1 | 0 | 2257 |
| docs/evidence/W-0043/README.md | W-0043 — Evidence: Docker images & dev compose (`P7-1`) | 1/46 | 0 | 3 | 0 | 6466 |
| docs/evidence/W-0044/README.md | W-0044 — Evidence: Kubernetes & Helm (`P7-2`) | 1/16 | 0 | 1 | 0 | 1807 |
| docs/evidence/W-0045/README.md | W-0045 — Evidence: CD pipeline & environment promotion (`P7-3`) | 1/8 | 0 | 0 | 0 | 1050 |
| docs/evidence/W-0046/README.md | W-0046 — Evidence: Progressive delivery & canary (`P7-4`) | 1/9 | 0 | 1 | 0 | 1063 |
| docs/evidence/W-0047/README.md | W-0047 — Evidence: Secret rotation & key lifecycle (`P7-5`) | 1/19 | 0 | 2 | 0 | 2195 |
| docs/evidence/W-0048/external-input-request.md | W-0048 — Phiếu lấy đầu vào Sales và one-SIM lab | 1/12 | 0 | 0 | 0 | 1209 |
| docs/evidence/W-0048/README.md | W-0048 — One-SIM lab + Sales integration readiness | 1/12 | 0 | 0 | 0 | 960 |
| docs/evidence/W-0052/README.md | W-0052 — Evidence: PDPA / privacy compliance & consent (`P10-1`) | 1/10 | 0 | 0 | 0 | 1346 |
| docs/evidence/W-0053/README.md | W-0053 — Evidence: Data governance, backup crypto & DR (`P10-2`) | 1/9 | 0 | 0 | 0 | 1596 |
| docs/evidence/W-0054/README.md | W-0054 — Evidence: Capacity, cost & SIM sizing model (`P10-3`) | 1/10 | 0 | 0 | 0 | 1607 |
| docs/evidence/W-0055/README.md | W-0055 — Evidence: Analytics / BI pipeline (`P10-4`) | 1/11 | 0 | 0 | 0 | 1931 |
| docs/evidence/W-0057/README.md | W-0057 — Evidence: Telephony Procurement Pack (`P11-1`) | 1/9 | 0 | 0 | 0 | 1160 |
| docs/evidence/W-0058/README.md | W-0058 — Evidence: Sales/Auth Target Contract Closure Pack (`P11-2`) | 1/10 | 0 | 0 | 0 | 1254 |
| docs/evidence/W-0059/README.md | W-0059 — Evidence: Legal retention & DF-03 sign-off pack (`P11-3`) | 1/9 | 0 | 0 | 0 | 1017 |
| docs/evidence/W-0060/README.md | W-0060 — Evidence: Production readiness command center (`P11-4`) | 1/8 | 0 | 0 | 0 | 935 |
| docs/evidence/W-0061/README.md | W-0061 — GitLab platform provisioning evidence | 1/16 | 0 | 0 | 0 | 2007 |
| docs/evidence/W-0064/README.md | W-0064 — P1-5 Retention Job & Data Lifecycle Evidence | 1/5 | 1 | 1 | 0 | 464 |
| docs/evidence/W-0065/README.md | W-0065 — P2-8 internal and admin API evidence | 1/9 | 0 | 0 | 0 | 1195 |
| docs/evidence/W-0066/README.md | W-0066 — P2-9 speech/TTS provider boundary evidence | 1/8 | 0 | 2 | 0 | 679 |
| docs/evidence/W-0066/test-report.md | W-0066 required test matrix | 1/1 | 1 | 0 | 0 | 245 |
| docs/evidence/W-0085/README.md | W-0085 Linux ProjectReference path portability | 1/6 | 0 | 0 | 0 | 270 |
| docs/evidence/W-0086/README.md | W-0086 — Shallow-clone Gitleaks fingerprint remediation | 1/4 | 0 | 0 | 0 | 401 |
| docs/evidence/W-0087/README.md | W-0087 — Phase 1/2 runtime continuity remediation | 1/4 | 0 | 0 | 0 | 191 |
| docs/evidence/W-0088/README.md | W-0088 — Phase 1/2 state-machine liveness remediation | 1/1 | 0 | 0 | 0 | 148 |
| docs/evidence/W-0089/README.md | W-0089 — Phase 1/2 privacy and intake correctness remediation | 1/1 | 0 | 0 | 0 | 201 |
| docs/evidence/W-0090/README.md | W-0090 — Phase 1/2 callback and admin atomicity remediation | 1/1 | 0 | 0 | 0 | 149 |
| docs/evidence/W-0091/README.md | W-0091 — Phase 1/2 retention and acceptance-test remediation | 1/1 | 0 | 0 | 0 | 213 |
| docs/evidence/W-0092/README.md | W-0092 — Phase 1/2 contract and configuration remediation | 1/1 | 0 | 0 | 0 | 257 |
| docs/evidence/W-0093/README.md | W-0093 — Phase 1/2 governance and evidence truth remediation | 1/5 | 0 | 0 | 0 | 601 |
| docs/evidence/W-0094/README.md | W-0094 — Phase 1/2 lifecycle and dead-code remediation | 1/1 | 0 | 0 | 0 | 194 |
| docs/evidence/W-0095/README.md | W-0095 — Admin read API (`GET /dashboard`, `/call-jobs`, `/call-jobs/{id}/detail`) | 1/8 | 0 | 0 | 0 | 999 |
| docs/evidence/W-0096/README.md | W-0096 — Back-office read API (`/scripts`, `/integration-status`, `/review-items`) | 1/8 | 0 | 0 | 0 | 835 |
| docs/evidence/W-0097/README.md | W-0097 — Admin UI visual design pass | 1/9 | 0 | 0 | 1 | 974 |
| docs/evidence/W-0098/README.md | W-0098 — Analytics read API (`/analytics/summary\|trend\|breakdown\|export`) | 1/10 | 0 | 0 | 0 | 1151 |
| docs/evidence/W-0099/README.md | W-0099 — SIM channel surface (`GET /sim-channels` + console controls) | 1/6 | 0 | 0 | 0 | 688 |
| docs/evidence/W-0100/README.md | W-0100 — Phase 3 audit hygiene remediation | 1/7 | 0 | 0 | 0 | 1022 |
| docs/evidence/W-0101/README.md | W-0101 — Phase 3 spec conformance (UI-01 / UI-02 / UI-03) | 1/8 | 0 | 0 | 0 | 888 |
| docs/evidence/W-0102/README.md | W-0102 — Phase 3 §10 live evidence capture | 1/8 | 0 | 0 | 0 | 1070 |
| docs/evidence/W-0103/README.md | W-0103 — Current-system audit closure | 1/8 | 1 | 1 | 0 | 877 |
| docs/evidence/W-0104/README.md | W-0104 — Free Asterisk/softphone telephony preflight | 1/6 | 0 | 0 | 0 | 677 |
| docs/gameday-report.md | Game-day report — IVR Order Confirmation (`W-0042` · `P6-3`) | 1/12 | 1 | 0 | 0 | 1506 |
| docs/integration-guide.md | IVR Integration Guide — Order Core, Ops and CRM | 1/12 | 0 | 0 | 0 | 644 |
| docs/kpi-catalog.md | KPI catalog — `W-0055` · `P10-4` | 1/11 | 0 | 0 | 0 | 1648 |
| docs/lab/one-sim-lab-plan.md | Kế hoạch lab — 1 SIM thật + đơn hàng mock | 1/28 | 0 | 0 | 0 | 3351 |
| docs/operations/gitlab-runner-winhost.md | GitLab self-hosted Docker runner on Windows | 1/8 | 0 | 0 | 0 | 676 |
| docs/owner-decisions-open.md | Quyết định đã chốt | 1/6 | 0 | 0 | 0 | 1059 |
| docs/perf-security-report.md | Performance, capacity and security/privacy report | 1/8 | 1 | 0 | 0 | 922 |
| docs/progressive-delivery.md | Progressive delivery — `W-0046` · `P7-4` | 1/7 | 1 | 0 | 0 | 1034 |
| docs/release/df03-signoff-input.md | DF-03 sign-off input — `W-0059` · `P11-3` | 1/8 | 0 | 0 | 0 | 986 |
| docs/release/readiness-board.md | Production readiness board — `W-0060` · `P11-4` | 1/8 | 0 | 0 | 0 | 735 |
| docs/reports/2026-08-15-bao-cao-tien-do-ivr.md | Báo cáo tiến độ — IVR Order Confirmation | 1/19 | 0 | 0 | 0 | 4100 |
| docs/review/2026-08-14-phase-1-2-code-review.md | Rà soát Phase 1 + Phase 2 — IVR Order Confirmation | 1/26 | 0 | 0 | 0 | 3454 |
| docs/review-checklist.md | IVR merge-request review checklist | 1/7 | 2 | 2 | 0 | 582 |
| docs/reviewer-guide.md | Reviewer guide — cái máy không bắt được | 1/10 | 2 | 1 | 0 | 918 |
| docs/secret-inventory.md | Secret inventory — `W-0047` · `P7-5` §6.1 | 1/6 | 1 | 0 | 0 | 696 |
| docs/secret-rotation-runbook.md | Secret rotation runbook — `W-0047` · `P7-5` §6.4 | 1/8 | 1 | 0 | 0 | 1038 |
| docs/slo.md | SLO / SLI — IVR Order Confirmation (`W-0041` · `P6-2`) | 1/14 | 0 | 0 | 0 | 2096 |
| docs/traceability-tests.md | Test traceability — test ID to source | 1/3 | 0 | 0 | 0 | 3429 |
| integration-requirements/00-index.md | Integration Requirements — Index (IVR Order Confirmation) | 1/4 | 0 | 5 | 0 | 224 |
| integration-requirements/01-sales-platform-requirements.md | IR-01 — Sales Platform / Order Core Requirements | 1/8 | 1 | 0 | 0 | 692 |
| integration-requirements/02-ops-core-requirements.md | IR-02 — Ops-Core Requirements (Module 1/2) | 1/3 | 1 | 0 | 0 | 457 |
| integration-requirements/03-telephony-sim-requirements.md | IR-03 — Telephony / SIM Gateway Requirements | 1/5 | 1 | 0 | 0 | 421 |
| integration-requirements/04-shared-auth-audit-requirements.md | IR-04 — Shared Auth, Audit and Release Requirements | 1/3 | 1 | 0 | 0 | 265 |
| integration-requirements/05-open-contract-questions.md | IR-05 — Open Contract Questions and Closure Evidence | 1/3 | 1 | 0 | 0 | 455 |
| plan/ivr-orther/_archive/01-reading-inventory.md | 01 — Reading Inventory | 1/7 | 0 | 0 | 0 | 1938 |
| plan/ivr-orther/_archive/02-current-understanding.md | 02 — Current Understanding | 1/15 | 1 | 2 | 0 | 1078 |
| plan/ivr-orther/_archive/03-ivr-related-findings.md | 03 — IVR-Related Findings | 1/12 | 0 | 2 | 0 | 769 |
| plan/ivr-orther/_archive/04-module-dependency-map.md | 04 — Module Dependency Map | 1/7 | 0 | 2 | 0 | 1223 |
| plan/ivr-orther/_archive/05-sales-platform-analysis-plan.md | 05 — Sales Platform Analysis Plan (Module 3 / 3.1) | 1/9 | 0 | 3 | 0 | 503 |
| plan/ivr-orther/_archive/06-ops-core-analysis-plan.md | 06 — Ops-Core Analysis Plan (Module 1 / 2) | 1/9 | 0 | 4 | 0 | 504 |
| plan/ivr-orther/_archive/07-source-of-truth-build-plan.md | 07 — Source-of-Truth Build Plan  ⭐ (file quan trọng nhất) | 1/10 | 0 | 1 | 0 | 776 |
| plan/ivr-orther/_archive/08-target-specs-structure-proposal.md | 08 — Target Specs Structure Proposal | 1/7 | 0 | 0 | 0 | 597 |
| plan/ivr-orther/_archive/09-specs-generation-sequence.md | 09 — Specs Generation Sequence | 1/16 | 2 | 0 | 0 | 705 |
| plan/ivr-orther/_archive/10-integration-gap-analysis.md | 10 — Integration Gap Analysis | 1/8 | 1 | 1 | 0 | 1364 |
| plan/ivr-orther/_archive/11-sales-platform-api-needs-draft.md | 11 — Sales Platform API Needs (DRAFT) | 1/12 | 2 | 0 | 0 | 950 |
| plan/ivr-orther/_archive/12-ops-core-api-needs-draft.md | 12 — Ops-Core API Needs (DRAFT) | 1/8 | 3 | 1 | 0 | 730 |
| plan/ivr-orther/_archive/13-seed-and-mock-strategy-plan.md | 13 — Seed & Mock Strategy Plan | 1/9 | 2 | 2 | 0 | 744 |
| plan/ivr-orther/_archive/15-open-questions.md | 15 — Open Questions | 1/13 | 2 | 1 | 0 | 1079 |
| plan/ivr-orther/_archive/16-prompt-roadmap.md | 16 — Prompt Roadmap | 1/9 | 1 | 6 | 0 | 1036 |
| plan/ivr-orther/_archive/prompts/00-index.md | Prompts Index — IVR-Orther | 1/5 | 1 | 16 | 0 | 572 |
| plan/ivr-orther/_archive/prompts/p01-generate-docs-review.md | P01 — Generate Docs Review | 1/9 | 1 | 0 | 0 | 373 |
| plan/ivr-orther/_archive/prompts/p02-generate-context-and-scope.md | P02 — Generate Context & Scope | 1/9 | 1 | 0 | 0 | 336 |
| plan/ivr-orther/_archive/prompts/p03-generate-functional-srs.md | P03 — Generate Functional SRS | 1/9 | 1 | 0 | 0 | 337 |
| plan/ivr-orther/_archive/prompts/p04-generate-workflows.md | P04 — Generate Workflows | 1/9 | 1 | 0 | 0 | 314 |
| plan/ivr-orther/_archive/prompts/p05-generate-api-specs.md | P05 — Generate API Specs | 1/9 | 1 | 0 | 0 | 339 |
| plan/ivr-orther/_archive/prompts/p06-generate-data-mapping.md | P06 — Generate Data Mapping | 1/9 | 1 | 0 | 0 | 242 |
| plan/ivr-orther/_archive/prompts/p07-generate-database-design.md | P07 — Generate Database Design | 1/9 | 1 | 0 | 0 | 302 |
| plan/ivr-orther/_archive/prompts/p08-generate-architecture-design.md | P08 — Generate Architecture Design | 1/9 | 1 | 0 | 0 | 302 |
| plan/ivr-orther/_archive/prompts/p09-generate-integration-requirements.md | P09 — Generate Integration Requirements | 1/9 | 1 | 0 | 0 | 309 |
| plan/ivr-orther/_archive/prompts/p10-generate-seed-data.md | P10 — Generate Seed Data | 1/9 | 1 | 0 | 0 | 299 |
| plan/ivr-orther/_archive/prompts/p11-generate-testing-specs.md | P11 — Generate Testing Specs | 1/9 | 1 | 0 | 0 | 265 |
| plan/ivr-orther/_archive/prompts/p12-generate-ui-specs.md | P12 — Generate UI Specs (Admin/Ops Console) | 1/9 | 1 | 0 | 0 | 231 |
| plan/ivr-orther/_archive/prompts/p13-generate-final-prompt-library.md | P13 — Generate Final Prompt Library | 1/9 | 1 | 0 | 0 | 226 |
| plan/ivr-orther/_archive/prompts/p14-review-and-normalize-specs.md | P14 — Review & Normalize Specs | 1/9 | 1 | 0 | 0 | 290 |
| plan/ivr-orther/00-index.md | IVR Order Confirmation — Plan Index (canonical) | 1/6 | 1 | 3 | 0 | 379 |
| plan/ivr-orther/14-risk-register.md | 14 — Risk Register | 1/2 | 0 | 1 | 0 | 1166 |
| plan/ivr-orther/decisions-log.md | Decisions Log — IVR (lịch sử quyết định + Target V1 overlay) | 1/13 | 16 | 3 | 0 | 3478 |
| plan/ivr-orther/production-blockers-plan.md | External Closure Plan — IVR Order Confirmation | 1/9 | 1 | 0 | 0 | 595 |
| plan/ivr-orther/questions-to-crm-3.1-followup.md | Câu hỏi bổ sung IVR — gửi CRM / Module 3.1 (vòng 2: do-not-call / opt-out / thông báo sau gọi) | 1/10 | 1 | 1 | 0 | 1161 |
| plan/ivr-orther/questions-to-module-3-and-3.1.md | Câu hỏi tích hợp IVR — gửi Team Module 3 (Commerce Order Core) & Module 3.1 (Sales Extensions) | 3/20 | 0 | 1 | 0 | 2197 |
| plan/ivr-orther/questions-to-ops-core.md | Câu hỏi tích hợp IVR — gửi Team Ops-Core (Module 1 Product Master / Module 2 Operational Core) | 3/15 | 0 | 1 | 0 | 1711 |
| plan/ivr-orther/questions-to-order-core-state.md | Yêu cầu bàn giao Order-State Contract — gửi Order Core (Module 3) | 1/9 | 0 | 1 | 0 | 985 |
| plan/ivr-orther/questions-to-telephony-and-foundation.md | Câu hỏi tích hợp IVR — gửi Telephony/Infra (SIM Gateway) & Foundation | 3/19 | 0 | 1 | 0 | 1659 |
| plan/ivr-orther/target-contract-v1-draft.md | Target Contract V1 Draft — IVR ↔ Sales Platform | 1/14 | 5 | 0 | 0 | 1749 |
| prompt/_execution/defaults-and-confirmations.md | Defaults and Confirmations — IVR Execution | 1/5 | 1 | 0 | 0 | 722 |
| prompt/_execution/prompt-execution-tracker.md | IVR Master Implementation Progress Ledger | 1/10 | 3 | 0 | 0 | 41310 |
| prompt/_legacy-mock/00-index.md | IVR Dev Prompt Library — Index | 1/4 | 0 | 9 | 0 | 369 |
| prompt/_legacy-mock/01-foundation-and-contracts.md | DEV PROMPT 01 — Foundation & Contracts | 1/10 | 1 | 0 | 0 | 240 |
| prompt/_legacy-mock/02-m8-2a-task-intake.md | DEV PROMPT 02 — M8.2A Task Intake | 1/9 | 1 | 0 | 0 | 190 |
| prompt/_legacy-mock/03-m8-2b-eligibility.md | DEV PROMPT 03 — M8.2B Eligibility Resolver | 1/9 | 1 | 0 | 0 | 161 |
| prompt/_legacy-mock/04-m8-2c-scheduler-queue.md | DEV PROMPT 04 — M8.2C Scheduler & Queue | 1/9 | 1 | 0 | 0 | 183 |
| prompt/_legacy-mock/05-m8-2d-sim-adapter.md | DEV PROMPT 05 — M8.2D SIM Gateway Adapter | 1/9 | 1 | 0 | 0 | 165 |
| prompt/_legacy-mock/06-m8-2e-dtmf-normalizer.md | DEV PROMPT 06 — M8.2E DTMF & Result Normalizer | 1/9 | 1 | 0 | 0 | 148 |
| prompt/_legacy-mock/07-m8-2f-order-core-callback.md | DEV PROMPT 07 — M8.2F Order Core Callback | 1/9 | 1 | 0 | 0 | 179 |
| prompt/_legacy-mock/08-m8-2g-admin-monitoring-ui.md | DEV PROMPT 08 — M8.2G Admin Monitoring / UI | 1/9 | 1 | 0 | 0 | 189 |
| prompt/_legacy-mock/09-m8-2h-smoke-evidence.md | DEV PROMPT 09 — M8.2H Smoke / Evidence Pack | 1/10 | 1 | 0 | 0 | 235 |
| prompt/_review/phase-0-11-spec-alignment-review.md | REVIEW — Phase 0-11 Spec Alignment | 1/7 | 0 | 0 | 0 | 663 |
| prompt/_review/zero-to-production-prompt-readiness-review.md | REVIEW — Zero-to-Production Prompt Readiness | 1/7 | 0 | 0 | 0 | 647 |
| prompt/_TEMPLATE.md | PROMPT {ID} — {Tên slice} | 1/16 | 0 | 0 | 0 | 857 |
| prompt/00-index.md | IVR Prompt Library — Master Index | 1/10 | 1 | 59 | 0 | 783 |
| prompt/phase-0-foundation/P0-1-repo-and-solution-bootstrap.md | PROMPT P0-1 — Repo & Solution Bootstrap | 1/14 | 1 | 0 | 0 | 913 |
| prompt/phase-0-foundation/P0-2-ci-baseline-quality-gates.md | PROMPT P0-2 — CI Baseline & Quality Gates | 1/14 | 1 | 0 | 0 | 2112 |
| prompt/phase-0-foundation/P0-3-crosscutting-foundation.md | PROMPT P0-3 — Cross-Cutting Foundation | 1/14 | 1 | 0 | 0 | 1083 |
| prompt/phase-0-foundation/P0-4-feature-flag-config-platform.md | PROMPT P0-4 — Feature-Flag & Dynamic Config Platform | 1/14 | 1 | 0 | 0 | 1098 |
| prompt/phase-1-contracts-data/P1-1-openapi-codegen-contract-scaffold.md | PROMPT P1-1 — Target/Compat OpenAPI, Codegen and Contract Scaffold | 1/8 | 1 | 0 | 0 | 306 |
| prompt/phase-1-contracts-data/P1-2-database-migrations-postgres.md | PROMPT P1-2 — PostgreSQL/EF Core Persistence for Target V1 | 1/7 | 1 | 0 | 0 | 227 |
| prompt/phase-1-contracts-data/P1-3-domain-model-dto-mapping.md | PROMPT P1-3 — Domain Model, Policies, Speech and Provider Ports | 1/7 | 1 | 0 | 0 | 217 |
| prompt/phase-1-contracts-data/P1-4-api-docs-developer-portal.md | PROMPT P1-4 — API Docs & Developer Portal | 1/14 | 1 | 0 | 0 | 559 |
| prompt/phase-1-contracts-data/P1-5-retention-job-data-lifecycle.md | PROMPT P1-5 — Retention Job & Data Lifecycle | 1/15 | 1 | 0 | 0 | 963 |
| prompt/phase-10-compliance-maturity/P10-1-pdpa-privacy-compliance.md | PROMPT P10-1 — PDPA / Privacy Compliance & Consent | 1/14 | 1 | 0 | 0 | 682 |
| prompt/phase-10-compliance-maturity/P10-2-data-governance-backup-dr.md | PROMPT P10-2 — Data Governance, Backup Crypto & DR Topology | 1/14 | 1 | 0 | 0 | 596 |
| prompt/phase-10-compliance-maturity/P10-3-capacity-cost-sim-sizing.md | PROMPT P10-3 — Capacity, Cost & SIM Sizing Model | 1/14 | 1 | 0 | 0 | 668 |
| prompt/phase-10-compliance-maturity/P10-4-analytics-bi-pipeline.md | PROMPT P10-4 — Analytics / BI Pipeline | 1/14 | 1 | 0 | 0 | 548 |
| prompt/phase-10-compliance-maturity/P10-5-sla-error-budget-oncall.md | PROMPT P10-5 — SLA, Error-Budget Policy & On-Call Maturity | 1/14 | 1 | 0 | 0 | 655 |
| prompt/phase-11-production-closure/P11-1-telephony-procurement-rfq-lab-acceptance.md | PROMPT P11-1 — Telephony RFQ, One-SIM Lab and 32-eSIM Closure | 1/7 | 1 | 0 | 0 | 272 |
| prompt/phase-11-production-closure/P11-2-cross-team-contract-closure-pack.md | PROMPT P11-2 — Sales/Auth Target Contract Closure Pack | 1/7 | 1 | 0 | 0 | 274 |
| prompt/phase-11-production-closure/P11-3-legal-retention-df03-signoff-pack.md | PROMPT P11-3 — Legal Retention & DF-03 Sign-Off Pack | 1/14 | 1 | 0 | 0 | 726 |
| prompt/phase-11-production-closure/P11-4-production-readiness-command-center.md | PROMPT P11-4 — Production Readiness Command Center | 1/7 | 1 | 0 | 0 | 270 |
| prompt/phase-2-core-runtime/P2-1-task-intake.md | PROMPT P2-1 — Target V1 Task Intake | 1/9 | 1 | 0 | 0 | 376 |
| prompt/phase-2-core-runtime/P2-2-eligibility-blockers.md | PROMPT P2-2 — Eligibility & Blockers | 1/14 | 1 | 0 | 0 | 683 |
| prompt/phase-2-core-runtime/P2-3-scheduler-attempt-policy.md | PROMPT P2-3 — Versioned Scheduler, Attempts and Channel Leases | 1/6 | 1 | 0 | 0 | 217 |
| prompt/phase-2-core-runtime/P2-4-sim-adapter-mock.md | PROMPT P2-4 — Mock Telephony, Dial Token and Speech Execution | 1/6 | 1 | 0 | 0 | 213 |
| prompt/phase-2-core-runtime/P2-5-dtmf-normalizer.md | PROMPT P2-5 — DTMF Normalizer | 1/14 | 1 | 0 | 0 | 556 |
| prompt/phase-2-core-runtime/P2-6-order-core-callback.md | PROMPT P2-6 — Sales Result Callback, Outbox and Compatibility Adapter | 1/6 | 1 | 0 | 0 | 198 |
| prompt/phase-2-core-runtime/P2-7-script-content-management.md | PROMPT P2-7 — Script, Content Approval and Speech Safety | 1/6 | 1 | 0 | 0 | 285 |
| prompt/phase-2-core-runtime/P2-8-internal-admin-api.md | PROMPT P2-8 — IVR Internal & Admin API Implementation | 1/15 | 1 | 0 | 0 | 1320 |
| prompt/phase-2-core-runtime/P2-9-speech-tts-provider.md | PROMPT P2-9 — Speech Rendering & TTS Provider Boundary | 1/15 | 1 | 0 | 0 | 1275 |
| prompt/phase-3-admin-ui/P3-1-ui-foundation.md | PROMPT P3-1 — Admin UI Foundation (Next.js) | 1/14 | 1 | 0 | 0 | 658 |
| prompt/phase-3-admin-ui/P3-2-dashboard-calllog-detail.md | PROMPT P3-2 — Dashboard, Call Log & Call Detail | 1/14 | 1 | 0 | 0 | 593 |
| prompt/phase-3-admin-ui/P3-3-config-integration-roles.md | PROMPT P3-3 — Config, Integration Status, Seed/Mock & Roles UI | 1/14 | 1 | 0 | 0 | 673 |
| prompt/phase-3-admin-ui/P3-4-reporting-analytics-ui.md | PROMPT P3-4 — Reporting & Analytics UI | 1/14 | 1 | 0 | 0 | 815 |
| prompt/phase-4-integration/P4-1-order-core-wiring.md | PROMPT P4-1 — Sales Platform Provider Wiring and Contract Verification | 1/8 | 1 | 0 | 0 | 353 |
| prompt/phase-4-integration/P4-2-ops-sellable-gate.md | PROMPT P4-2 — Sales Eligibility/Blocker Snapshot Contract | 1/7 | 1 | 0 | 0 | 270 |
| prompt/phase-4-integration/P4-3-crm-eligibility-events.md | PROMPT P4-3 — Voice Call Restriction and Trust Snapshot | 1/7 | 1 | 0 | 0 | 274 |
| prompt/phase-4-integration/P4-4-shared-auth-audit.md | PROMPT P4-4 — Target Service JWT, Optional mTLS and Audit | 1/7 | 1 | 0 | 0 | 256 |
| prompt/phase-4-integration/P4-5-post-decision-notification.md | PROMPT P4-5 — V1 Notification Disabled Boundary | 1/5 | 1 | 0 | 0 | 178 |
| prompt/phase-4-integration/P4-6-opt-out-feedback-loop.md | PROMPT P4-6 — Opt-Out Feedback Loop | 1/14 | 1 | 0 | 0 | 626 |
| prompt/phase-5-quality/P5-1-unit-integration-tests.md | PROMPT P5-1 — Unit & Integration Test Suite | 1/14 | 1 | 0 | 0 | 677 |
| prompt/phase-5-quality/P5-2-contract-e2e-tests.md | PROMPT P5-2 — Contract & E2E Test Suite | 1/14 | 1 | 0 | 0 | 767 |
| prompt/phase-5-quality/P5-3-performance-security-tests.md | PROMPT P5-3 — Performance, Load & Security/Privacy Tests | 1/14 | 1 | 0 | 0 | 617 |
| prompt/phase-5-quality/P5-4-code-review-gate.md | PROMPT P5-4 — Code Review Gate & Static Analysis | 1/14 | 1 | 0 | 0 | 654 |
| prompt/phase-5-quality/P5-5-accessibility-i18n-qa.md | PROMPT P5-5 — Accessibility, i18n & Cross-Browser QA | 1/14 | 1 | 0 | 0 | 535 |
| prompt/phase-6-observability/P6-1-logging-metrics-tracing.md | PROMPT P6-1 — Logging, Metrics & Tracing (OpenTelemetry) | 1/14 | 1 | 0 | 0 | 595 |
| prompt/phase-6-observability/P6-2-dashboards-slo-alerting.md | PROMPT P6-2 — Dashboards, SLO & Alerting | 1/14 | 1 | 0 | 0 | 527 |
| prompt/phase-6-observability/P6-3-chaos-resilience-gamedays.md | PROMPT P6-3 — Chaos & Resilience Game-Days | 1/14 | 1 | 0 | 0 | 671 |
| prompt/phase-7-deployment/P7-1-docker-images-compose.md | PROMPT P7-1 — Docker Images & Dev Compose | 1/14 | 1 | 0 | 0 | 571 |
| prompt/phase-7-deployment/P7-2-kubernetes-helm.md | PROMPT P7-2 — Kubernetes & Helm | 1/14 | 1 | 0 | 0 | 662 |
| prompt/phase-7-deployment/P7-3-cicd-pipeline.md | PROMPT P7-3 — CD Pipeline & Environment Promotion | 1/14 | 1 | 0 | 0 | 772 |
| prompt/phase-7-deployment/P7-4-progressive-delivery-canary.md | PROMPT P7-4 — Progressive Delivery & Canary | 1/14 | 1 | 0 | 0 | 568 |
| prompt/phase-7-deployment/P7-5-secret-rotation-key-lifecycle.md | PROMPT P7-5 — Secret Rotation & Key Lifecycle | 1/14 | 1 | 0 | 0 | 697 |
| prompt/phase-8-sim-pilot/P8-1-real-sim-adapter.md | PROMPT P8-1 — Vendor Adapter and One-Real-SIM Lab | 1/7 | 1 | 0 | 0 | 296 |
| prompt/phase-8-sim-pilot/P8-2-pilot-runbook.md | PROMPT P8-2 — Real-SIM Lab Runbook (No Customers) | 1/7 | 1 | 0 | 0 | 285 |
| prompt/phase-9-release-ops/P9-1-release-gate-execution.md | PROMPT P9-1 — Production Customer-Call Release Gate | 1/9 | 1 | 0 | 0 | 380 |
| prompt/phase-9-release-ops/P9-2-cutover-ops-runbook.md | PROMPT P9-2 — Cutover, Rollback & Ops Runbook | 1/14 | 1 | 0 | 0 | 730 |
| prompt/README-governance.md | IVR Prompt Library — Governance (đọc trước mọi prompt) | 1/10 | 1 | 0 | 0 | 1096 |
| prompt/RUNBOOK-execute-prompts.md | RUNBOOK — Execute IVR Prompts P0–P11 | 1/10 | 1 | 0 | 0 | 432 |
| README.md | GinsengFood IVR | 1/7 | 0 | 1 | 0 | 642 |
| seed/README.md | IVR Seed / Mock Data (NON-PRODUCTION ONLY) | 1/7 | 0 | 0 | 0 | 617 |
| specs/_review/normalization-report.md | REVIEW — Normalization Report | 1/28 | 0 | 0 | 0 | 1439 |
| specs/_review/open-decisions-register.md | REVIEW — Open Decisions Register | 1/5 | 5 | 0 | 0 | 1035 |
| specs/_review/traceability-matrix.md | REVIEW — Target V1 Traceability Matrix | 1/1 | 0 | 0 | 0 | 464 |
| specs/01-context-and-scope.md | SRS-01 — Context and Scope | 1/6 | 1 | 0 | 0 | 335 |
| specs/02-business-goals.md | SRS-02 — Business Goals | 1/5 | 0 | 1 | 0 | 376 |
| specs/03-stakeholders-and-actors.md | SRS-03 — Stakeholders & Actors | 1/4 | 0 | 0 | 0 | 491 |
| specs/04-glossary.md | SRS-04 — Glossary | 1/1 | 1 | 0 | 0 | 780 |
| specs/05-current-docs-review.md | SRS-05 — Current Docs Review (IVR Order Confirmation) | 1/14 | 0 | 0 | 0 | 2894 |
| specs/06-assumptions-and-open-questions.md | SRS-06 — Assumptions and Open Questions | 1/4 | 0 | 1 | 0 | 252 |
| specs/api/00-index.md | API SRS — Index | 1/2 | 0 | 8 | 0 | 160 |
| specs/api/01-conventions.md | API-01 — Conventions | 1/5 | 2 | 0 | 0 | 255 |
| specs/api/02-internal-api.md | API-02 — IVR-Owned Internal API | 1/4 | 1 | 1 | 0 | 376 |
| specs/api/03-admin-api.md | API-03 — Admin API | 1/8 | 1 | 0 | 0 | 727 |
| specs/api/04-sim-adapter-contract.md | API-04 — SIM Adapter Contract (Internal — Adapter Port) | 1/6 | 3 | 0 | 0 | 542 |
| specs/api/05-order-core-contracts.md | API-05 — Sales Platform Contracts | 1/6 | 2 | 0 | 0 | 312 |
| specs/api/06-error-codes.md | API-06 — Error Codes | 1/9 | 1 | 0 | 0 | 880 |
| specs/api/07-idempotency-and-correlation.md | API-07 — Idempotency & Correlation | 1/5 | 1 | 0 | 0 | 382 |
| specs/api/08-external-api-needs.md | API-08 — External API Needs | 1/4 | 1 | 0 | 0 | 136 |
| specs/architecture/00-index.md | Architecture SRS — Index | 1/1 | 0 | 7 | 0 | 98 |
| specs/architecture/01-system-context.md | ARCH-01 — System Context | 1/1 | 1 | 0 | 0 | 43 |
| specs/architecture/02-module-boundaries.md | ARCH-02 — IVR Module Boundaries | 1/1 | 1 | 0 | 0 | 179 |
| specs/architecture/03-integration-architecture.md | ARCH-03 — Integration Architecture | 1/1 | 1 | 0 | 0 | 167 |
| specs/architecture/04-deployment-architecture.md | ARCH-04 — Deployment Architecture | 1/2 | 1 | 0 | 0 | 83 |
| specs/architecture/05-resilience.md | ARCH-05 — Resilience (Failure / Fail-closed / Retry / Cache) | 1/7 | 1 | 0 | 0 | 514 |
| specs/architecture/06-observability.md | ARCH-06 — Observability & Ops | 1/7 | 1 | 0 | 0 | 318 |
| specs/architecture/07-diagrams.md | ARCH-07 — Summary Diagrams | 1/3 | 1 | 0 | 0 | 26 |
| specs/data/00-index.md | Data SRS — Index | 1/4 | 0 | 6 | 0 | 232 |
| specs/data/01-data-ownership.md | DATA-01 — Data Ownership | 1/1 | 1 | 0 | 0 | 172 |
| specs/data/02-mapping-sales-platform.md | DATA-02 — Mapping: Sales Platform ↔ IVR | 1/4 | 1 | 0 | 0 | 258 |
| specs/data/03-mapping-ops-core.md | DATA-03 — Mapping: Ops-Core (Sellable Gate / Sale-Lock / Recall) | 1/7 | 1 | 0 | 0 | 427 |
| specs/data/04-missing-data.md | DATA-04 — Missing External Data and Contracts | 1/1 | 1 | 0 | 0 | 203 |
| specs/data/05-pii-policy.md | DATA-05 — PII & Privacy Policy | 1/7 | 5 | 0 | 0 | 524 |
| specs/database/00-index.md | Database SRS — Index | 1/1 | 0 | 6 | 0 | 101 |
| specs/database/01-erd.md | DB-01 — ERD | 1/2 | 1 | 0 | 0 | 97 |
| specs/database/02-tables.md | DB-02 — Tables | 1/12 | 1 | 0 | 0 | 2565 |
| specs/database/03-enums-and-status.md | DB-03 — Enums and Status | 1/1 | 1 | 0 | 0 | 117 |
| specs/database/04-indexes.md | DB-04 — Indexes & Constraints | 1/6 | 1 | 0 | 0 | 1012 |
| specs/database/05-retention-and-privacy.md | DB-05 — Retention & Privacy | 1/8 | 2 | 2 | 0 | 1090 |
| specs/database/06-migration-plan.md | DB-06 — Migration Plan | 1/7 | 2 | 0 | 0 | 384 |
| specs/decisions/DF-07-retention-policy.md | DF-07 — Retention policy | 1/5 | 0 | 0 | 0 | 531 |
| specs/decisions/DT-05-recording-off-policy.md | DT-05 — Recording OFF policy | 1/6 | 0 | 0 | 0 | 511 |
| specs/decisions/DT-06-blocked-result-semantics.md | DT-06 — Semantics của operational/policy block | 1/4 | 3 | 0 | 0 | 321 |
| specs/functional/00-index.md | Functional SRS — Index | 1/4 | 0 | 8 | 0 | 379 |
| specs/functional/01-task-intake.md | FR — Task Intake | 1/3 | 1 | 0 | 0 | 282 |
| specs/functional/02-eligibility-and-blockers.md | FR — Eligibility and Blockers | 1/1 | 1 | 0 | 0 | 169 |
| specs/functional/03-scheduler-attempt-policy.md | FR — Scheduler and Attempt Policy | 1/3 | 1 | 0 | 0 | 211 |
| specs/functional/04-call-execution-dtmf.md | FR — Call Execution, Speech and DTMF | 1/5 | 1 | 0 | 0 | 400 |
| specs/functional/05-result-normalization-callback.md | FR — Result Normalization and Sales Callback | 1/5 | 1 | 1 | 0 | 328 |
| specs/functional/06-technical-exception-capacity.md | FR — Technical Exception & Capacity | 1/5 | 1 | 0 | 0 | 641 |
| specs/functional/07-admin-operations.md | FR — Admin / Ops Operations | 1/4 | 1 | 0 | 0 | 548 |
| specs/functional/08-evidence-audit-privacy.md | FR — Evidence, Audit & Privacy | 1/4 | 1 | 0 | 0 | 410 |
| specs/tech/00-tech-stack.md | TECH-STACK — Standalone IVR Service | 1/5 | 0 | 0 | 0 | 230 |
| specs/testing/00-index.md | Testing SRS — Index | 1/1 | 0 | 9 | 0 | 122 |
| specs/testing/01-strategy.md | TEST-01 — Strategy | 1/6 | 1 | 0 | 0 | 308 |
| specs/testing/02-unit-test-plan.md | TEST-02 — Unit and Property Test Plan | 1/1 | 1 | 0 | 0 | 152 |
| specs/testing/03-integration-test-plan.md | TEST-03 — Integration Test Plan | 1/1 | 1 | 0 | 0 | 118 |
| specs/testing/04-contract-test-plan.md | TEST-04 — Contract Test Plan | 1/4 | 1 | 0 | 0 | 102 |
| specs/testing/05-e2e-test-plan.md | TEST-05 — End-to-End Plan | 1/1 | 1 | 0 | 0 | 145 |
| specs/testing/06-performance-test-plan.md | TEST-06 — Performance and Capacity Test Plan | 1/1 | 1 | 0 | 0 | 128 |
| specs/testing/07-security-privacy-test-plan.md | TEST-07 — Security & Privacy Test Plan | 1/6 | 1 | 0 | 0 | 324 |
| specs/testing/08-acceptance-criteria.md | TEST-08 — Acceptance Criteria and Gates | 1/6 | 1 | 0 | 0 | 295 |
| specs/testing/09-smoke-matrix.md | TEST-09 — Target V1 Smoke Matrix | 1/1 | 1 | 0 | 0 | 263 |
| specs/ui/00-index.md | UI SRS — Index (Admin/Ops Console) | 1/4 | 0 | 8 | 0 | 247 |
| specs/ui/01-dashboard.md | UI-01 — Dashboard (Queue / Capacity / Incident) | 1/6 | 1 | 0 | 0 | 142 |
| specs/ui/02-call-log.md | UI-02 — Call Log (danh sách CallJob) | 1/7 | 1 | 0 | 0 | 115 |
| specs/ui/03-call-detail.md | UI-03 — Call Detail (trace task→job→attempt→result→callback) | 1/6 | 1 | 0 | 0 | 167 |
| specs/ui/04-ivr-menu-config.md | UI-04 — IVR Menu / Script Config | 1/6 | 2 | 0 | 0 | 283 |
| specs/ui/05-integration-status.md | UI-05 — Integration Status (Health) | 1/6 | 1 | 0 | 0 | 112 |
| specs/ui/06-callback-request.md | UI-06 — Admin Review & Technical Retry | 1/5 | 1 | 0 | 0 | 180 |
| specs/ui/07-seed-mock-management.md | UI-07 — Seed / Mock Management (NON-PROD only) | 1/5 | 1 | 0 | 0 | 135 |
| specs/ui/08-role-permission-ui.md | UI-08 — Role / Permission & Matrix | 1/6 | 1 | 0 | 0 | 391 |
| specs/workflows/00-index.md | Workflows — Index | 1/1 | 0 | 9 | 0 | 119 |
| specs/workflows/01-happy-path-confirm.md | Workflow — Happy Path Confirm (key 1) | 1/1 | 1 | 0 | 0 | 31 |
| specs/workflows/02-cancel.md | Workflow — Cancel (phím 0) | 1/1 | 1 | 0 | 0 | 50 |
| specs/workflows/03-no-answer-attempts.md | Workflow — No Answer and Timeout | 1/1 | 1 | 0 | 0 | 47 |
| specs/workflows/04-invalid-phone.md | Workflow — Invalid Phone | 1/1 | 1 | 0 | 0 | 102 |
| specs/workflows/05-technical-exception.md | Workflow — Technical Exception | 1/1 | 1 | 0 | 0 | 67 |
| specs/workflows/06-race-condition-revalidation.md | Workflow — Race Condition and Sales Revalidation | 1/1 | 1 | 0 | 0 | 36 |
| specs/workflows/07-trusted-skip.md | Workflow — Trusted Customer Skip | 1/1 | 1 | 0 | 0 | 127 |
| specs/workflows/08-capacity-hold.md | Workflow — Capacity Hold / Incident | 1/1 | 1 | 0 | 0 | 79 |
| specs/workflows/09-state-machines.md | Workflow — IVR State Machines | 1/3 | 1 | 0 | 0 | 66 |

