...
| 22    | (sửa 28/08, `W-0137`) Bản `.docx` cùng tên **không** là bản này | Giải nén `.docx` đọc thẳng nội dung: text chỉ `45.807` ký tự so với `101.773` byte của bản `.md`; chữ **"VoLTE" xuất hiện 0 lần**; **"GSM Gateway" còn 2 lần**; **0 dòng errata 27/08 hoặc 28/08**. Tức bản `.docx` thiếu trọn đợt rà soát 27/08 và vẫn mang yêu cầu thiết bị 2G đã bị bác ở mục 21. **Bản `.md` này là bản có hiệu lực.** Không tài liệu nào trong repo tham chiếu bản V0.3 `.docx` — mọi tham chiếu hiện có đều trỏ bản V0.2. Thu hồi hay tái sinh bản `.docx` là quyết định của Owner: `OD-20`. |
