...
| 22    | (sửa 28/08, `W-0137`; thực thi 29/08, `W-0141`) Bản `.docx` cùng tên **không** là bản này | Giải nén `.docx` đọc thẳng nội dung: text chỉ `45.807` ký tự so với `101.773` byte của bản `.md`; chữ **"VoLTE" xuất hiện 0 lần**; **"GSM Gateway" còn 2 lần**; **0 dòng errata 27/08 hoặc 28/08**. Tức bản `.docx` thiếu trọn đợt rà soát 27/08 và vẫn mang yêu cầu thiết bị 2G đã bị bác ở mục 21. **Bản `.md` này là bản có hiệu lực.** Module 8 / Project Owner đã ký `OD-20=OPTION_1_WITHDRAW`; W-0141 đã đổi tên artifact lỗi thời thành `MODULE_8_IVR_ORDER_CONFIRMATION_V0.3_CLEAN_SUPERSEDED.docx`, giữ nguyên SHA-256 `b2b95c9cb62e14b8138538b8447117040207641e5c565e4e1881f3a55af0935c`. Tên `_SUPERSEDED` là cảnh báo bắt buộc, không phải một bản Word hiện hành. |
...
| B-09   | Mười hai file audio đoạn cố định chưa ai nghe thử. Ba giọng vùng miền được chốt dựa trên mô tả văn bản, không dựa trên nghe.                                                                   | THEO DÕI       | Owner + Product           | Owner nghe đủ ba giọng và ký; nghe mối nối giữa đoạn VieNeu render sẵn và đoạn tổng hợp lúc gọi trong một cuộc gọi thật. OD-VOICE-01, OD-VOICE-05.                                       |
...
| M3 Decision Gate    | Task chỉ được phát sau khi Module 3 đã quyết định `CALL_REQUIRED`; IVR không phân loại lại trusted/non-trusted. | Reject task hoặc không dispatch; không tự suy ra trusted skip. |
...
Module 3 là business authority và chỉ phát task sau khi đã quyết định đơn cần gọi. Resolver phía IVR chỉ kiểm tra kỹ thuật đối với snapshot từ Order Core, Official Contact, Operational Block và Program Policy; không đọc trust metadata để đảo quyết định gọi/không gọi, không tự bỏ qua recall/sale lock và không tự kéo dài window.
...
| risk_flags / legacy trust metadata         | Module 3 task payload             | Không tạo trusted skip. `risk_flags` chỉ dùng cho audit và IVR-derived scheduler priority; `customer_trust_status`/`trusted_skip_allowed` chỉ `LEGACY_READ` cho compatibility/history. |
...
4. Đơn có `RiskScore` nội bộ cao hơn, được IVR suy từ `risk_flags` phục vụ xếp hàng; tiêu chí này không được đổi quyết định call/skip của Module 3.
