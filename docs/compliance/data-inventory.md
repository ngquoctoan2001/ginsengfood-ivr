...
| `ivr_idempotency_keys.payload_hash`, `ivr_task_intake_outbox.payload_sha256`, và khoá `payload_sha256` trong `data_json` của bản ghi audit intake (`ivr_audit_log`) | SHA-256 của **toàn bộ** body intake đã chuẩn hoá, để nhận ra lần gửi lại cùng nội dung. Body có `phone_e164` khi M3 gửi số (`W-0310`), nên hash là **dữ liệu giả danh hoá** của số đó: ai có đủ các field còn lại của đơn có thể thử lần lượt các số và so hash. Cổng theo tên cột (§5) không bắt được, vì tên cột không nói gì về số *(thêm `25/09`, `W-0356`)* | hợp đồng | DSAR **không** chạm tới. Dòng idempotency và outbox xoá theo retention (`created_at`); bản trong audit **không bao giờ** xoá (§4 nhóm 1) |
