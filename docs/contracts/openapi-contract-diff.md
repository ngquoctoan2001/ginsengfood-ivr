...
- SHA-256: `9b810b4fe7f97170c0602c9bbb61047a04e77db7a111eb6157de8130b7ec6e54`
- Title/version: IVR Order Confirmation — Internal/Admin API (Target V1 Draft) / `1.0.0-draft.15`
- Generated: `src/Ivr.Contracts/Generated/IvrServer/V1/IvrServerModels.g.cs`
- Operations (49): POST /tasks (intakeTask); POST /eligibility-checks (recordEligibility); GET /call-jobs (listCallJobs); POST /call-jobs (createCallJob); GET /call-jobs/{ivrCallJobId} (getCallJob); POST /call-attempts (recordAttempt); POST /call-results (recordResult); POST /result-callbacks (recordResultCallback); GET /queue (getQueue); GET /dashboard (getDashboard); GET /call-jobs/{ivrCallJobId}/detail (getCallJobDetail); GET /sim-channels (listSimChannels); GET /scripts (getScriptCatalog); GET /scripts/{templateId}/{version} (getScriptVersion); POST /scripts/ (createScriptDraft); POST /scripts/{templateId}/{version}:submit (submitScriptForReview); POST /scripts/{templateId}/{version}:approve (approveScriptVersion); POST /scripts/{templateId}/{version}:retire (retireScriptVersion); POST /dev/seed:load (loadDevSeed); POST /dev/scenarios/{scenarioId}:dry-run (dryRunDevScenario); POST /dev/integration-profiles/{profileId}:apply (applyDevIntegrationProfile); POST /call-jobs:terminate-all (terminateAllCallJobs); POST /call-jobs/{ivrCallJobId}:terminate (terminateCallJob); GET /integration-status (getIntegrationStatus); GET /review-items (listReviewItems); GET /analytics/summary (getAnalyticsSummary); GET /analytics/trend (getAnalyticsTrend); GET /analytics/breakdown (getAnalyticsBreakdown); GET /analytics/export (exportAnalytics); POST /queue:pause (pauseQueue); POST /queue:resume (resumeQueue); POST /sim-channels/{simChannelId}:disable (disableSim); POST /sim-channels/{simChannelId}:enable (enableSim); POST /technical-retries (technicalRetry); POST /admin-reviews (adminReview); GET /feature-flags/{environment} (getFeatureFlags); POST /feature-flags/{environment} (mutateFeatureFlags); GET /feature-flags/{environment}/kill-switch (verifyFeatureFlagKillSwitch); POST /auth/sign-in (signInConsoleAccount); GET /auth/session (getConsoleSession); POST /auth/sign-out (signOutConsoleAccount); GET /accounts (listConsoleAccounts); POST /accounts (createConsoleAccount); GET /accounts/me (getMyConsoleAccount); GET /accounts/{accountId} (getConsoleAccount); PATCH /accounts/{accountId} (updateConsoleAccount); DELETE /accounts/{accountId} (deleteConsoleAccount); POST /accounts/{accountId}:reset-password (resetConsoleAccountPassword); GET /account-roles (getConsoleAccountRoles)
- Schemas (93): AdminMutationRequest, AdminReviewRequest, CallAttemptLifecycleRequest, CallJobLifecycleRequest, CallResultLifecycleRequest, ConsoleAccount, ConsoleAccountErrorCode, ConsoleAccountErrorEnvelope, ConsoleAccountPage, ConsoleAccountStatus, ConsoleRole, ConsoleRoleMatrix, ConsoleSession, ConsoleSignInRequest, ConsoleSignInResult, ConsoleSignOutResult, CreateConsoleAccountRequest, DeleteConsoleAccountRequest, EligibilityLifecycleRequest, ErrorCode, ErrorEnvelope, FeatureFlagChangeSet, FeatureFlagMutationRequest, FeatureFlagMutationResult, FeatureFlagReadResult, FeatureFlagSnapshot, IvrAdminActionResult, IvrAdminReviewResult, IvrAnalyticsBreakdown, IvrAnalyticsBreakdownRow, IvrAnalyticsDataQuality, IvrAnalyticsExport, IvrAnalyticsFilter, IvrAnalyticsKpi, IvrAnalyticsSummary, IvrAnalyticsTrend, IvrAnalyticsTrendBucket, IvrCallAttempt, IvrCallAttemptDetail, IvrCallJob, IvrCallJobDetail, IvrCallJobListItem, IvrCallJobPage, IvrCallResult, IvrCallResultDetail, IvrCapacityIncidentSummary, IvrConfirmationTaskV1, IvrDashboardAttemptPanel, IvrDashboardProjection, IvrDashboardQueuePanel, IvrDashboardResultPanel, IvrDashboardSimPanel, IvrDependencyStatus, IvrDtmfKey, IvrEligibilityDecision, IvrFailClosedEvent, IvrIntegrationProfileEffect, IvrIntegrationProfileResult, IvrIntegrationStatus, IvrQueueProjection, IvrResultCallbackDetail, IvrResultCallbackLifecycle, IvrReviewItemDetail, IvrReviewQueue, IvrReviewQueueItem, IvrScenarioAttemptReplay, IvrScenarioDryRunResult, IvrScriptActionResult, IvrScriptApproval, IvrScriptApprovalRequest, IvrScriptCatalog, IvrScriptDraftRequest, IvrScriptTransitionRequest, IvrScriptVersion, IvrScriptVersionDetail, IvrSeedLoadRequest, IvrSeedLoadResult, IvrSeedTaskOutcome, IvrSimChannel, IvrSimChannelList, IvrTaskIntakeResult, IvrTechnicalExceptionDetail, IvrTechnicalRetryResult, KillSwitchVerification, OrderSpeechItem, PrivacySafeOrderSummary, ProgramCode, ResetConsolePasswordRequest, ResultCallbackLifecycleRequest, ResultType, SellableStatusLine, TechnicalRetryRequest, UpdateConsoleAccountRequest
