...
Due: chốt **trước khi bắt đầu `P4-1`** — provider wiring không viết được khi chưa có endpoint generic. Ngày cam kết của owner: `<owner điền>`.
...
- `ResultType`: **11 giá trị** (`IVR_CONFIRMED`, `IVR_CUSTOMER_CANCELLED`, `IVR_NO_ANSWER_ATTEMPT`, `IVR_NO_ANSWER_FINAL`, `IVR_CONFIRMATION_WINDOW_EXPIRED`, `IVR_INVALID_PHONE_FINAL`, `IVR_WRONG_INPUT`, `IVR_TECHNICAL_EXCEPTION`, `IVR_CAPACITY_EXCEPTION`, `IVR_OPERATIONAL_BLOCKED`, `IVR_POLICY_BLOCKED`)
...
**(b) 11 → 4 là ánh xạ mất mát, và mất đúng thứ quan trọng.** Trên đường compat, `IVR_NO_ANSWER_ATTEMPT` và `IVR_NO_ANSWER_FINAL` đều phải gập vào `NO_ANSWER`; `IVR_TECHNICAL_EXCEPTION`, `IVR_CAPACITY_EXCEPTION`, `IVR_OPERATIONAL_BLOCKED`, `IVR_POLICY_BLOCKED` đều gập vào `FAILED`. Sales mất khả năng phân biệt "khách không nghe máy lần 1" với "hết lượt, kết thúc" — và mất luôn `DT-02` (lỗi kỹ thuật không tính là lần gọi khách). Cần Sales xác nhận: chấp nhận mất mát này trên đường compat, và Target V1 giữ đủ 11.
...
WireMock phát đủ 6 ACK code + 6 lớp lỗi; outbox đã có test bền vững trên Postgres thật. Đường compat đã verify tại commit ghim — **`CURRENT_COMPAT_VERIFIED_AT_PINNED_SHA`**, không phải "đã tích hợp".

## 6. Closure artifact — owner điền

- [ ] **OpenAPI endpoint generic** từ Sales (path, body, ACK taxonomy), phủ cả hai chương trình.
- [ ] **Bảng ánh xạ 11 → 4** cho đường compat, có xác nhận Sales chấp nhận phần mất mát.
