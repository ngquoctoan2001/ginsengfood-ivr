...
**2. `OD-V1-15` và `OD-V1-19` ràng buộc nhau, chưa ai nối hai cái lại.** Whitelist lời thoại quyết định phương án audio: bộ hẹp (mã đơn + số tiền) có thể ghép audio dựng sẵn, rẻ và không có dữ liệu rời mạng nội bộ; bộ rộng (thêm tên sản phẩm và khu vực giao) gần như buộc phải có TTS động, kéo theo PDPA và chi phí theo ký tự. Hai quyết định đang nằm ở hai owner khác nhau và chưa có ai nói cho nhau biết. Đã ghi ở R-05 §1.
