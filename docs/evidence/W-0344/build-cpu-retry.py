#!/usr/bin/env python3
"""Build retry patch r2: the r1 portable Asterisk image, byte for byte, plus the RTP-gated case driver.

r1 (22/09 14:06) replaced only the Asterisk image and was never sent to S5. Its second local
rehearsal pressed DTMF on a wall-clock estimate while a starved 0.5-CPU PBX was still playing,
so r2 also carries deploy/lab/full-flow-s5/cases.py. Nothing else may differ from the S5 base kit.
...
NAME = 'ivr-full-flow-w0344-cpu-r2.tar.gz'
...
def main():
    ...
    with tarfile.open(R1) as r1:
        ...
        if manifest != expected:
            raise SystemExit('r2 manifest would change more than the image and case driver')
        ...
        change = {'work_id': 'W-0344', 'REAL_CUSTOMER_CALL_ALLOWED': 'NO',
                  ...
                  'supersedes': {'archive': R1.name, 'sha256': R1_SHA256, 'sent_to_s5': False}}
    ...
    print('W0344_CPU_R2_BUILT bytes=%d sha256=%s' % (target.stat().st_size, file_sha256(target)), flush=True)
