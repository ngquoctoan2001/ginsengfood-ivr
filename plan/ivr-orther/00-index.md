...
`_archive/`, các file `questions-to-*` và `prompt/_legacy-mock/` là lịch sử/reference, không phải nguồn điều khiển implementation mới. Phiếu [questions-to-module-3-od15-risk-evidence.md](questions-to-module-3-od15-risk-evidence.md) là `SUPERSEDED` bởi `OD-18`/`W-0123`; Module 3 không còn nợ trust/risk-evidence field cho IVR tự skip. Lượt rà soát `W-0123` và phần khắc phục của nó nằm ở [W-0124-w0123-review-remediation-plan.md](W-0124-w0123-review-remediation-plan.md); `W-0124` xoay baseline so sánh OpenAPI sang `draft.20` nhưng **giữ** baseline `draft.2` và báo cáo chuyển tiếp của nó làm lịch sử audit. Nhãn dùng: `CURRENT_COMPAT`, `TARGET_DRAFT`, `OWNER_DECISION_REQUIRED`, `BLOCKED_EXTERNAL`, `IMPLEMENTED`, `VERIFIED`.
