...
| **DC-06** | `HISTORICAL_CONTEXT`: Module 3 chưa có `CustomerTrustResolver`; điều này không còn tạo nghĩa vụ tích hợp với IVR. `OD-18` đặt toàn bộ phân loại khách và business call/no-call trong Module 3; IVR không đọc trust/risk-evidence để quyết định. Placement trung gian `OD-15` là `SUPERSEDED`. | QC6 / Q-F3 → **không còn gap phía IVR (`OD-18`)** |
...
| **OD-15** | **HISTORICAL / SUPERSEDED PLACEMENT bởi `OD-18`; nội dung quyết định gốc được giữ nguyên sau đây.** **Không gọi IVR cho khách cũ.** Bằng chứng "khách cũ" = Sales gửi `eligibility_snapshot.trust.risk_evidence_available=true` **và** `risk_flags` **rỗng** → `TASK_SKIPPED_TRUSTED_CUSTOMER`, không tạo CallJob. **Bỏ** yêu cầu `customer_trust_status=TRUSTED` và `resolver_available` khỏi predicate (D-12 cũ) — khách mới đã tự mang cờ `NEW_CUSTOMER`/`VERIFIED_ORDER_COUNT_0` nên một phép kiểm "list rỗng" trả lời cả hai vế, **không** phải chờ `CustomerTrustResolver` mà DC-06 ghi là chưa build. **Giữ nguyên** ngoại lệ D-12: bất kỳ risk flag nào (COD fail, nghi trùng, địa chỉ/phone rủi ro, giá trị bất thường, Giờ Vàng rủi ro) → **vẫn gọi**. `trusted_skip_allowed=false` = **veto** của Sales cho đơn đó; absent = im lặng, không veto. `trust.resolver_version` fallback về `source_version` cấp snapshot. **Fail-closed không đổi:** `risk_evidence_available` thiếu/false → list rỗng **không** được đọc là "không rủi ro" → vẫn gọi. Cờ chính sách `IVR_RETURNING_CUSTOMER_SKIP_ENABLED` mặc định **ON**, đặt `NO` để rollback. **Wire contract shape KHÔNG đổi:** ba field `customer_trust_status`/`trusted_skip_allowed`/`risk_flags` giữ nguyên kiểu và vẫn optional; chỉ *nghĩa* của `trusted_skip_allowed` đổi từ opt-in bắt buộc → **veto**. Nghĩa mới đã ghi thẳng vào OpenAPI: **`draft.18 → draft.19`** (owner duyệt cùng ngày) — thêm `description` cho ba field + enum `decision`, regenerate `IvrServerModels.g.cs` (chỉ thêm 12 dòng XML doc), cập nhật `contract-manifest.json`, contract-diff report, oasdiff changelog và docs portal. `oasdiff breaking` vs baseline `draft.2`: **no breaking changes**; số operation (49) và schema (93) không đổi, nên rolling deploy giữa service không bị ảnh hưởng. | `SUPERSEDED` placement — owner 2026-08-27 qua `OD-18`; quyết định gốc owner 2026-08-25 từng supersede **OD-08** và phần trust-score của **D-12** | `docs/evidence/W-0118/README.md`; OpenAPI historical draft.19; UT-ELIG-TRUST-16/18/19 |
| **OD-17** | **Gỡ `sellable_status[]` khỏi IVR hoàn toàn.** IVR không đọc tồn kho/thu hồi/sale-lock/quality-hold từ bất kỳ nguồn nào; **không còn đường dữ liệu nào giữa IVR và ops-core**. Xoá khỏi contract (`draft.19 → draft.20`, breaking), domain (`EligibilitySellableLine`, `EligibilitySellableDecision`, `EvaluateSellable`, 10 reason code), intake, admin read API, console (bảng chi tiết cuộc gọi + i18n), seed, và thẻ dependency `OPS_SELLABLE_GATE`. **Lý do:** có ba tầng kiểm nhưng chỉ tầng ba quyết định — `eligibility_snapshot.decision` (M3, trước khi đẩy task), `sellable_status[]` (IVR, trước khi quay số), và **`D-06` revalidate với ops (M3, lúc callback)**. Tầng hai chỉ tránh được một cuộc gọi thừa, không tránh được một xác nhận sai, vì `D-06` vẫn chạy bất kể IVR có chặn hay không. Gỡ nó xoá luôn nghĩa vụ fan-out `order → SKU/batch → availability/check` của M3. **Đánh đổi owner chấp nhận:** IVR có thể gọi khách về đơn vừa bị recall trong cửa sổ 5–15′; khách bấm `1` rồi M3 trả `BLOCKED_BY_CORE`. **Rủi ro còn lại:** `D-06` nay là lưới an toàn **duy nhất** — M3 bỏ bước revalidate thì không còn gì chặn xác nhận đơn không bán được, và IVR không phát hiện được vì đã không nhìn thấy dữ liệu đó. **Cột DB `sellable_status_json`/`sellable_captured_at` giữ lại** (mồ côi): gate `W-0114` cấm `DropColumn` trong `Up()` vì `helm rollback` đi qua chiều code-cũ-trên-schema-mới. | ✅ LOCKED — owner 2026-08-26; supersede `IR-02` toàn bộ, thay hệ quả `DO-01`/`DO-02`/`DO-05` với IVR | `EligibilityRules.cs`; `specs/api/openapi/*` `draft.20`; `integration-requirements/{02,06}`; `specs/data/03-mapping-ops-core.md` (xoá); `prompt/phase-4-integration/P4-2-ops-sellable-gate.md` (xoá) |
| **OD-18** | **M3 quyết định nghiệp vụ; IVR chỉ thực thi.** M3 chỉ gửi task sau khi đã quyết định đơn cần gọi. IVR không phân loại lại khách cũ/khách mới và không dùng `customer_trust_status`, `trusted_skip_allowed`, `eligibility_snapshot.trust.risk_evidence_available` hoặc empty/non-empty `risk_flags` để đảo quyết định gọi/không gọi. IVR vẫn fail-closed ở contract/auth/idempotency, official order/state, eligibility evidence, transactional do-not-call, phone/token, privacy, window, script/policy và capacity. `risk_flags` được phép giữ cho audit/scheduler priority, không phải eligibility. `TASK_SKIPPED_TRUSTED_CUSTOMER` và các cột DB liên quan chỉ còn `LEGACY_READ` cho history/rollback; runtime mới không emit/write. Vì M3 usage và target DB count chưa có evidence, cutover đầu **deprecate/ignore, không drop** để giữ rolling compatibility. | ✅ LOCKED — owner 2026-08-27; supersede **placement** của `OD-15`, không phủ nhận business outcome “khách M3 xác định không cần gọi thì không gọi” | `W-0123`; `integration-requirements/06-module-3-api-handover.md`; `docs/evidence/W-0123/README.md` |
...
- **CRM / Module 3.1 (vòng 2):** ✅ đã trả lời (DC-01..06). **Việc build còn lại:** (a) DC-01 bổ sung read-contract cho `crm-ads-eligibility` (trả `do_not_call/opt_out_scope/reason/effective_at`); (b) DC-05 implement event `ORDER_CONFIRMED/CANCELLED/EXPIRED` sau Core decision + CRM notification. Trust resolver/risk-evidence không còn là gap tích hợp IVR theo `OD-18`; phiếu `OD-15` cũ là `SUPERSEDED`.
