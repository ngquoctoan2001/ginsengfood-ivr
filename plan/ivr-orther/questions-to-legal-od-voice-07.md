...
Điều đó **đã vào code**: `TargetV1SpeechPolicy.cs` template `v3-test-approved` không còn
`{{customer_display_name}}`. Còn ba placeholder động — `{{items_spoken}}`,
`{{total_amount_display}}`, `{{delivery_area_short}}` — trong đó hai cái sau thu trước được, còn
`items_spoken` (tên hàng) thì hữu hạn theo catalog nhưng phải thu lại khi catalog đổi.

**Chưa ai trả lời** `items_spoken` có thu trước không. Hệ quả cho phiếu này:

| Câu hỏi | Nếu **bỏ** VieNeu lúc chạy | Nếu **giữ** VieNeu |
| --- | --- | --- |
| `L1` `L2` `L3` `L4` — licence model, training data, quyền 3 preset, attribution | vẫn cần **nếu** 12 đoạn cố định đang dùng là bản do VieNeu render; **không cần** nếu thu lại bằng giọng người | **cần đủ** |
| `L5` `L6` `L7` — nghĩa vụ khi bỏ SaaS, retention, lab tạm thời | **cần dù đường nào** | **cần** |

> **Điểm dễ bỏ sót:** kể cả khi VieNeu không còn chạy lúc gọi, **12 đoạn cố định hiện có vẫn là
> audio do VieNeu tổng hợp** và Owner đã ký ngày `28/08`. Nên `L3` (quyền thương mại của đúng ba
> preset) **vẫn áp dụng cho chính những file đó**, trừ khi chúng được thu lại bằng giọng người.
...
| Trả lời | Hành động |
| --- | --- |
| `L1`–`L3` đều chấp nhận | Ghi vào `MODELS.lock.legal_gate` đúng **năm** trường dưới đây; đóng `OD-VOICE-07`; các gate khác vẫn mở |
...
### Đúng năm trường gate sẽ kiểm

Gate `tts-provenance-gate.mjs` và `verify-model.py` cùng đòi cả năm; thiếu một là `PASS` bị
**từ chối**, không phải bỏ qua:

| Trường | Giá trị bắt buộc |
| --- | --- |
| `status` | `PASS` |
| `decision_authority` | **`LEGAL_PRIVACY`** — chuỗi chính xác |
| `decided_by` | tên người/bộ phận ký, không rỗng |
| `approval_reference` | số hiệu tra lại được, không rỗng |
| `decided_on` | **đúng dạng `YYYY-MM-DD`** |

> ⚠️ **Chữ ký của Owner module IVR bị từ chối bằng code, không phải bằng quy trình.** Ngày
> `2026-08-28`, commit `2a4f45d` đặt `legal_gate = PASS` ký *"Owner module IVR"*; `TODAY-03` phải
> khôi phục. Nay có mutation test `legal-authority` khẳng định `decision_authority=MODULE_8_OWNER`
> **luôn** bị từ chối. Nghĩa là phiếu này không thể được đóng từ bên trong dự án — đó là chủ ý.
