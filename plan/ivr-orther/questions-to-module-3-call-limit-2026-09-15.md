...
> ### ⚠️ Khôi phục `17/09` (`W-0309`) — phiếu này từng bị xoá nhầm
> `W-0297` dồn `25` tài liệu vào một index và **xoá cả `6` phiếu nhóm A**, trong đó có phiếu này.
> Index khi đó vẫn ghi *“đã soạn xong, chỉ còn việc gửi”* — nhưng không còn gì để gửi. Đã khôi phục
> nguyên văn từ lịch sử git (`257cbef^`).
>
> **Đây là phiếu nhóm A duy nhất có người nhận thật.** `5` phiếu còn lại gửi cho Platform / Security /
> Legal — ba phòng ban **không tồn tại** trong tổ chức này; chúng đã được gom về
> [phiếu quyết định cho Sếp](phieu-quyet-dinh-cho-sep-2026-09-17.md) hoặc đóng lại.
>
> **Nội dung không sửa một chữ.** Đã kiểm lại tại `main@fbe6edb`: `AttemptPolicyRegistries.cs` vẫn
> giữ đúng hai lịch nêu trong bảng dưới (`GOLDEN_HOUR` `0s`/`150s` cửa sổ `300s`;
> `TWENTY_FOUR_SEVEN` `0s`/`450s` cửa sổ `900s`), nên cả `3` câu vẫn còn nguyên hiệu lực.
