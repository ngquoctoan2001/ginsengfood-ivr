...
Lý do ký (`od-v1-signoff §2.1`): chỉ **tên khách** là vô hạn; mã đơn và tiền là chữ số ghép được từ
ngân hàng ghi âm (`W-0108` đã dựng cơ chế), vùng giao là tập hữu hạn phường/quận thu trước được.

Và điều đó **đã vào code**: `TargetV1SpeechPolicy.cs` template `v3-test-approved` **không còn**
`{{customer_display_name}}`. Còn đúng ba placeholder động:

| Placeholder | Thu trước được không? |
| --- | --- |
| `{{total_amount_display}}` | **được** — chữ số |
| `{{delivery_area_short}}` | **được** — tập hữu hạn phường/quận |
| `{{items_spoken}}` | **tuỳ** — tên hàng; hữu hạn theo catalog GinsengFood, nhưng phải thu lại mỗi lần catalog đổi |

**Nên câu chưa ai trả lời là:** `items_spoken` có thu trước theo catalog không, và ai duy trì ngân
hàng ghi âm đó khi thêm sản phẩm?

- **Nếu có** → không cần TTS lúc chạy → **mục A (mirror 201 MiB weights) không còn cần**. Chỉ trả
  lời B và C.
- **Nếu không** → VieNeu vẫn chạy lúc gọi → trả lời cả ba mục.
...
> **Định dạng bắt buộc** (gate kiểm bằng regex, `W-0225`):
> `internal_mirror_uri` — chuỗi không rỗng · `internal_mirror_digest` — `^(sha256:)?[a-f0-9]{64}$`

#### `INF-A4` — Hồ sơ quyết định cho `internal_mirror_gate` (P1) · **mới `2026-09-08`**

Trước `W-0225`, cổng này mở được bằng đúng ba chữ — `{"status": "PASS"}` — và `INTERNAL_MIRROR`
rời khỏi danh sách blocker **trong khi cả 13 `internal_mirror_uri` vẫn `null`**. Nay không còn:
gate đòi cả hồ sơ lẫn bằng chứng, và từ chối nếu thiếu một trong hai.

Xin điền, cùng lúc với `INF-A2`:

| Trường | Giá trị |
| --- | --- |
| `decided_by` | `_______________________` (tên người/nhóm chịu trách nhiệm) |
| `approval_reference` | `_______________________` (số ticket/tài liệu tra lại được) |
| `decided_on` | `__________` — **đúng dạng `YYYY-MM-DD`** |

> `decision_authority` **cố ý không bị chỉ định**. Cổng legal buộc `LEGAL_PRIVACY` vì owner không
> được tự ký review pháp lý của mình; cổng này thì lý do trong `MODELS.lock` ghi thiếu
> *"**owner-approved** internal artifact or OCI mirror URI/digest"* — tức owner **là** thẩm quyền
> đúng. Đòi hồ sơ, không đòi danh tính.

> ⚠️ **Điền xong sẽ làm gate đỏ một lần, và đó là bình thường.** `expectedArtifactSetSha256` phủ cả
> hai trường mirror, nên giá trị thật làm fingerprint lệch và gate báo
> `artifact provenance fingerprint drift`. **Platform không làm hỏng gì cả** — đó là review point
> hoạt động. Team IVR sẽ re-pin cùng lượt theo dây chuyền `W-0126`.
...
| Trả lời | Hành động |
| --- | --- |
| `INF-A2` đủ 13 dòng **và** `INF-A4` đủ ba trường | Ghi vào `MODELS.lock`, re-pin `expectedArtifactSetSha256`, chạy `tts-provenance-gate` + `verify-model.py --mode production`; vẫn cần `L1` mới qua được |
| `INF-A2` đủ nhưng thiếu `INF-A4` | Gate **từ chối** — `internal mirror approval invalid`. Không ghi nửa vời vào lock |
