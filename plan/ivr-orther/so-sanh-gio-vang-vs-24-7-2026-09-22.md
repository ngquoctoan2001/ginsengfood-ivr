...
| 4 | ~~Kịch bản full-flow S5 (`deploy/lab/full-flow-s5/cases.py`) chỉ chạy `GOLDEN_HOUR`~~ *Sửa `23/09` (`W-0345`): `cases.py` chạy được cả hai chương trình qua `IVR_FLOW_PROGRAM`* | ~~Luồng 24/7 chưa từng được chạy end-to-end trên lab~~ *Đã chạy `23/09`: 7/7 ca `TWENTY_FOUR_SEVEN` + `COD`, local và S5, đối soát đúng. Vẫn ở mức 1 attempt/300 s của `lab-softphone-v1`, nên mục 3 còn nguyên* |
