...
| `K-56` | 🆕 (tìm thấy 26/09 lúc làm L7) | ARI: `ProcessEvent` chỉ bắt `JsonException`, một sự kiện sai kiểu làm chết vòng nhận sự kiện (từ `K-47` ít ra kết thúc cuộc gọi là `ASTERISK_EVENT_STREAM_LOST`); mất luồng giữa lúc quay và lúc phát bị ghi `ASTERISK_CHANNEL_ALREADY_ENDED` với kênh lành, không nêu tên việc mất luồng | `AsteriskAriSimGateway.cs` | test sự kiện sai kiểu; test mất luồng trước khi phát | 1 | ⬜ |
