...
> | II — Cần quyết định (`Q-01…Q-37`) | 37 — Toàn 19 · chief/Tech Lead 11 · Sếp 3 · M3/anh Mạnh 4 | ≈ 33 giờ sau khi Toàn quyết · ≈ 48 giờ sau khi chief/Tech Lead quyết (31 giờ là N1) |
...
| Mã | Nguồn | Việc | File chính | Kiểm | Giờ | Trạng thái |
| --- | --- | --- | --- | --- | ---: | --- |
| `K-42` | `B13` | Bù B13: `MockTelephonyDispatchGateway.cs:219, :226` dùng `executionContext.ToDomainMode()`; test kiến trúc `ARCH-DISPATCH-MODE-01` (cấm `ExecutionMode.LabRealSim/ProductionReal` trong `*DispatchGateway.cs`); `UT-AST-SCRIPT-01` (kịch bản thiếu phê duyệt Lab bị từ chối lúc dispatch, `DialAsync` không được gọi); phòng thủ kép ở `SpeechSynthesisService.cs:59-64` — coi là production khi tham số **hoặc** chế độ deployment là `PRODUCTION_REAL` (`UT-TTS-WHITELIST-08`); tuỳ chọn gom 3 bản đổi chuỗi→enum về `ExecutionModes.Parse` | Telephony, Speech | 3 test mới | 2,25 | ✅ b005144 · 1353/1353 (W-0362) · ARCH-DISPATCH-MODE-01, UT-TTS-WHITELIST-08, UT-AST-SCRIPT-01; không gom ExecutionModes.Parse (ba bản khác nhau về hoa thường) |
| `K-43` | 🆕 | `store.LoadAsync` nằm ngoài `try` ở cả 2 gateway (Asterisk `:58-64`, Mock `:207-213`) → lỗi lúc load bỏ lease treo: kênh `RESERVED` 120 s → `QUARANTINED` 600 s + 1 lỗi DT-04; job thành `HELD_LEASE_RECOVERY` → đưa vào `try`, `FailAsync` với kênh lành. Kết cục riêng cho task bị thu hồi làm trong gói C13 | 2 gateway | `IT-TEL-LOAD-FAIL-01` | 1,25 | ✅ b005144 · 1353/1353 (W-0362) · UT-AST-LOAD-FAIL-01, IT-TEL-LOAD-FAIL-01; kết cục task bị thu hồi vẫn ở C13 |
| `K-44` | 🆕 | Gate chỉ kiểm một lần, trước bước chuẩn bị audio (TTS tới 120 s) → bấm dừng khẩn cấp lúc đang tổng hợp giọng thì cuộc gọi vẫn đi → gọi lại `DispatchGate` sát `DialAsync` (kế hoạch SIP `:191` đã đòi) | `AsteriskSchedulerDispatchGateway.cs` | `UT-AST-GATE-03` | 1 | ✅ b005144 · 1353/1353 (W-0362) · UT-AST-GATE-03 |
| `K-45` | 🆕 | Kết cục `healthy=true` gọi `RecordHealthy` xoá `FailCount` dù kênh chưa hề được dùng (lỗi render, lỗi dữ liệu) → xoá luôn 2 lỗi SIM thật trước đó, kênh hỏng lại được ưu tiên → thêm kết cục "không đụng kênh" | `SimChannelFailurePolicy.cs:34-39`, dispatch store | test kênh có FailCount=2 | 1 | ✅ b005144 · 1353/1353 (W-0362) · IT-TEL-HEALTH-UNTOUCHED-09; "không đụng kênh" = kết cục lành khi chưa có cuộc gọi |
| `K-46` | 🆕 | Có store ghi thẳng `AuditLogEntity`, không qua `PostgresAuditLogger`/PiiGuard (`PostgresTelephonyDispatchStore.cs:519-535`, `PostgresSchedulerStore.cs:636`) → `PersistenceInvariantValidator` kiểm mọi `AuditLogEntity` | Persistence | test audit PII | 1 | ✅ b005144 · 1353/1353 (W-0362) · IT-DB-AUDIT-PII-10; khoá audit recording của dispatch store đổi thành recording_mode |
| `K-47` | 🆕 `V6-6` | Mất luồng sự kiện ARI: `ReceiveAsync` ném `WebSocketException` ra ngoài (`AsteriskAriSimGateway.cs:445-447, :475`), bỏ qua `FailOpenCalls` → cuộc đang mở rơi vào timeout, bị ghi là khách không nghe/không bấm **có tính lượt** → bọc `try/finally`, gọi `FailOpenCalls("ASTERISK_EVENT_STREAM_LOST")` ⇒ `NETWORK_ERROR`, `is_counted=false` | `AsteriskAriSimGateway.cs` | test WS server giả ngắt TCP giữa lúc chờ DTMF | 2 | ✅ b005144 · 1353/1353 (W-0362) · UT-AST-EVENTS-11/12; kèm cúp máy cuộc bị đóng vì mất luồng và giữ kết cục đã biết |
| `K-52` | 🆕 (tìm thấy 25/09 lúc làm L2) | Job bị eligibility giữ vì hết dung lượng (`CAPACITY_HELD`, `eligible=false` — `EligibilityRepository.cs:287-289`) **không bao giờ đóng và Module 3 không nhận callback nào**: `CAPACITY_HELD` chỉ được ghi, không nơi nào đọc lại (ngoài CHECK), còn sweep hết hạn chỉ quét job `eligible IS TRUE` ở 4 trạng thái (`PostgresSchedulerStore.cs:372-386`). Đơn 24/7 COD treo mà Module 3 không biết, trái bảng map (`IVR_CAPACITY_EXCEPTION` → người trực). Sửa: cho sweep đóng cả job `CAPACITY_HELD` khi hết cửa sổ với `IVR_CAPACITY_EXCEPTION` như nhánh capacityMiss (sửa đồng bộ bản chép vị từ ở `SchedulerQueueBacklog.cs`); `gitnexus_impact` `CloseMissedDeadlinesAsync` = HIGH. Làm cùng/sau gói C13 nếu chạm cùng câu SQL | `PostgresSchedulerStore.cs`, `SchedulerQueueBacklog.cs` | `IT-SCH-CAPACITY-HELD-01` (giữ ở eligibility → hết cửa sổ → một kết quả `IVR_CAPACITY_EXCEPTION` + callback) | 2 | ✅ b005144 · 1353/1353 (W-0362) · IT-SCH-CAPACITY-HELD-01; SchedulerQueueBacklog không cần đổi (sao điều kiện nhận job, không phải sweep) |
| `K-53` | 🆕 (tìm thấy 25/09 lúc làm L4) | `PiiSafeLogRecordProcessor` chỉ xuất thuộc tính có trong allowlist, và `ExceptionType` không có trong đó, nên tên loại lỗi của log hangup (`K-31`, W-0359) và của `FeatureFlagPlatform` không tới OTLP; dòng log chỉ còn `ReasonCode`, `AttemptId`. Thêm `ExceptionType` vào allowlist: giá trị là tên kiểu .NET, không phải dữ liệu khách | `src/Ivr.Infrastructure/Observability/IvrObservability.cs` | test processor giữ `ExceptionType` | 0,5 | ✅ b005144 · 1353/1353 (W-0362) · UT-OBS-LOG-14 |
| `K-54` | 🆕 (tìm thấy 26/09 lúc làm L7) | Eligibility còn kiểu giữ `TASK_HELD_ADMIN_REVIEW` (`eligible=false`) mà sweep hết hạn không đóng, như `CAPACITY_HELD` trước `K-52`; một job còn chờ eligibility khi cửa sổ đã hết cũng không đóng. Module 3 không nhận callback cho cả hai → mở rộng sweep cho hai trường hợp, kết cục theo bảng map (hết cửa sổ, cần người xem), cùng hàng rào exactly-once của `K-52` | `PostgresSchedulerStore.cs` | IT cho hai trường hợp | 1,5 | ✅ 4f9462e · 1388/1388 (W-0365) · IT-SCH-ADMIN-HELD-01, IT-SCH-PENDING-ELIG-01, IT-ELIG-CLOSED-01; eligibility khoá dòng job, worker nhận 409 thay 500; lời IR-06 thành K-59 |
| `K-55` | 🆕 (tìm thấy 26/09 lúc làm L7) | Phép kiểm text trên JSON audit (`PostgresAuditLogger`, `PersistenceInvariantValidator`) bỏ sót số dạng `+84` và dấu địa chỉ có dấu: `JsonSerializer` mặc định escape `+` và chữ có dấu thành `\u…`, nên guard không thấy → lọc trên giá trị đã giải mã; đo trước xem chữ tiếng Việt nào đang qua được sẽ bị từ chối | `PostgresAuditLogger.cs`, `PersistenceInvariantValidator.cs` | test audit với số `+84` và chữ có dấu | 1 | ✅ 4f9462e · 1388/1388 (W-0365) · UT-PII-JSON-01/02, IT-DB-AUDIT-PII-11; đo trên toàn suite trước khi chặn: không lối ghi nào bị từ chối thêm |
| `K-56` | 🆕 (tìm thấy 26/09 lúc làm L7) | ARI: `ProcessEvent` chỉ bắt `JsonException`, một sự kiện sai kiểu làm chết vòng nhận sự kiện (từ `K-47` ít ra kết thúc cuộc gọi là `ASTERISK_EVENT_STREAM_LOST`); mất luồng giữa lúc quay và lúc phát bị ghi `ASTERISK_CHANNEL_ALREADY_ENDED` với kênh lành, không nêu tên việc mất luồng | `AsteriskAriSimGateway.cs` | test sự kiện sai kiểu; test mất luồng trước khi phát | 1 | ✅ 4f9462e · 1388/1388 (W-0365) · UT-AST-EVENTS-13/14; mất luồng trước khi phát ghi kênh không lành như nhánh bắt phím, xem K-57 |
| `K-57` | 🆕 (tìm thấy 26/09 lúc làm K-56) | Mất luồng sự kiện ARI không phải lỗi của SIM, nhưng nhánh bắt phím (`GetDispositionAsync`, từ `K-47`) và nhánh phát (`PlayAsync`, `K-56`) đều báo kênh không lành, nên `FinalizeAsync` cộng một lỗi vào chuỗi lỗi SIM. Một luồng sự kiện dùng chung cho mọi SIM của worker: một lần chập mạng cộng lỗi cho mọi SIM đang có cuộc gọi, ba lần trong 10 phút là `HEALTH_FAILED`, và kênh Asterisk không có lối bật lại (bật lại chỉ nhận MOCK). Sửa: kết cục trung tính "không phải lỗi SIM" ở `FinalizeAsync` (không cộng, không xoá chuỗi lỗi), dùng cho cả hai nhánh. Cùng chỗ: raw event ghi audio `PLAYED` cho mọi lỗi sau khi quay không phải lỗi audio, kể cả khi phát bị từ chối | `AsteriskAriSimGateway.cs`, `PostgresTelephonyDispatchStore.cs`, `SimChannelFailurePolicy.cs` | test mất luồng không đổi chuỗi lỗi SIM, ở cả hai nhánh | 1,5 | ✅ 93e8e35 · 1408/1408 (W-0367) · UT-AST-HEALTH-01, IT-TEL-HEALTH-NEUTRAL-10, IT-TEL-AUDIO-STATUS-11, UT-TEL-PLAYBACK-01; UT-AST-EVENTS-11/14 nay chờ null; mất luồng không cộng cũng không xoá chuỗi lỗi SIM; Asterisk sập vẫn tính, xem K-62 |
| `K-58` | 🆕 (tìm thấy 26/09 lúc làm K-56) | `AsteriskAriSimGateway.DisposeAsync` gửi frame đóng rồi chờ trả lời không có timeout: đầu kia không trả lời thì worker treo lúc tắt (đúng cách `FakeAsterisk` treo trước `K-56`). Nó cũng chỉ nuốt `WebSocketException` của vòng nhận sự kiện; lỗi khác bị ném lại từ `DisposeAsync` | `AsteriskAriSimGateway.cs` | test dispose khi đầu kia im lặng; test dispose sau lỗi vòng nhận | 0,5 | ✅ c1cb722 · 1417/1417 (W-0369) · UT-AST-DISPOSE-01/02; đóng luồng trong 5 giây rồi huỷ, không ném lỗi của vòng nhận sự kiện |
| `K-59` | 🆕 (tìm thấy 26/09 lúc làm K-54) | IR-06 (gửi Module 3) nói sai sau `K-54`: bảng từ vựng trên dây ghi `TASK_HELD_ADMIN_REVIEW` là "im lặng", và khối bảng map ghi cặp `IVR_CONFIRMATION_WINDOW_EXPIRED` + `CORE_REVALIDATE_AND_HOLD_ADMIN_REVIEW` chỉ vì lỗi phía IVR. Nay đơn eligibility giữ chờ duyệt hoặc chưa kịp trả lời cũng nhận cặp đó khi hết cửa sổ; soát cùng lúc câu chữ về đơn bị giữ vì hết dung lượng (`K-52`, callback `IVR_CAPACITY_EXCEPTION`). Sửa ở lần ghim lại IR-06 kế tiếp, kèm chuỗi ghim, và báo Module 3 | `integration-requirements/06-module-3-api-handover.md` và chuỗi ghim của nó | các gate ghim hash | 0,5 | ✅ c1cb722 · 1417/1417 (W-0369) · IR-06 §3.11, §4.3 khối C22, §10 và IR-07 đính chính 26/09; ghim lại 4 validator, 3 template; báo M3 là việc của Toàn |
| `K-60` | 🆕 (tìm thấy 26/09 lúc làm K-54) | Eligibility chặn (`TASK_BLOCKED_OPERATIONAL`) đóng job mà không có kết quả hay callback (theo DT-06), trong khi intake đã bảo Module 3 chờ callback. Eligibility còn có thể chạy sau khi cửa sổ đã hết (tới 16 task tuần tự, timeout 10 giây mỗi task) rồi chặn vì hết cửa sổ, tranh với sweep của `K-54`: bên nào lấy khoá dòng trước quyết định Module 3 có nhận callback hay không. Có thể cần quyết định về hợp đồng: eligibility chặn có phát callback không, hay để sweep đóng mọi job hết cửa sổ | `EligibilityService.cs`, `EligibilityRepository.cs`, `PostgresSchedulerStore.cs` | IT eligibility chặn sau khi hết cửa sổ | 1 | ✅ c1cb722 · 1417/1417 (W-0369) · IT-ELIG-EXPIRED-01: lượt xét trễ bị từ chối (409), sweep đóng kèm callback; phần hợp đồng thành Q-37 (⏸ chief) |
| `K-61` | 🆕 (tìm thấy 26/09 lúc làm K-54) | Cảnh báo `IvrConfirmationDeadlineMissed` (`deploy/observability/alerts/ivr-slo.rules.yml`) cộng `ivr_missed_deadline_total` mọi lý do, và runbook (`docs/slo.md` §9) bảo hiệu chỉnh lại mô hình dung lượng. Sau `K-52` và `K-54`, đơn hết cửa sổ không vì thiếu kênh (giữ chờ duyệt, chưa được eligibility trả lời) cũng bật cảnh báo này → lọc theo lý do, hoặc tách hai cảnh báo | `ivr-slo.rules.yml`, `docs/slo.md`, `capacity-selftest.mjs` | self-test luật cảnh báo | 0,5 | ✅ c1cb722 · 1417/1417 (W-0369) · IT-SLO-CAPACITY-04, CAP-ALERT-04; cảnh báo dung lượng chỉ còn NO_DISPATCH_BEFORE_DEADLINE, thêm IvrConfirmationWindowsExpiringUndialled (ticket) |
| `K-62` | 🆕 (tìm thấy 26/09 lúc làm K-57) | Asterisk không truy cập được vẫn tính vào SIM ở lần quay sau: `ASTERISK_EVENT_STREAM_UNAVAILABLE` và `ASTERISK_HTTP_UNAVAILABLE` báo kênh không lành, nên mỗi SIM được thử khi Asterisk sập nhận một lỗi, ba lần trong 10 phút là `HEALTH_FAILED` không có lối trở lại (lối bật kênh của admin chỉ nhận MOCK). Lý lẽ của `K-57` áp cả ở đây, nhưng việc cách ly đang vô tình làm cầu dao: nó dừng dispatch khi Asterisk sập thay vì để mỗi job đốt lượt thử kỹ thuật. Cần quyết định: một cầu dao ở cấp worker không làm hỏng SIM, hoặc cho kênh Asterisk một lối trở lại | `AsteriskAriSimGateway.cs`, `PostgresTelephonyDispatchStore.cs`, `InternalAdminApiService.cs` | test Asterisk sập không đổi chuỗi lỗi SIM; test cầu dao | 1,5 | ✅ c1cb722 · 1417/1417 (W-0369) · UT-AST-UNAVAILABLE-01/02; PA1 theo khuyến nghị: Asterisk không truy cập được không tính vào SIM, cầu dao là backoff của pump; timeout HTTP còn ở K-63 |
| `K-63` | 🆕 (tìm thấy 26/09 lúc làm K-62) | Lớp HTTP của adapter ARI. Asterisk bị chặn gói (không từ chối kết nối) làm lời gọi hết hạn sau 100 giây, vì client ARI không đặt timeout; lỗi rơi vào nhánh chung `ASTERISK_DISPATCH_TECHNICAL_FAILURE`, vẫn tính vào SIM, và mỗi lần dispatch chờ tới 100 giây. Trả lời không phải 2xx: `503` tính vào SIM, mọi mã khác (kể cả `401`, `403`) coi là kênh lành và xoá chuỗi lỗi. Sửa: timeout ngắn cho client, ánh xạ timeout như không truy cập được (`K-62`), xét lại verdict theo từng mã HTTP | `AsteriskAriSimGateway.cs` (`SendAsync`, `EnsureSuccessAsync`), đăng ký HttpClient ARI | test Asterisk chặn gói; test verdict theo mã HTTP | 1 | ✅ 5c5b820 · 1476/1476 (W-0372) · UT-AST-HTTP-01…05; lệnh ARI và việc mở luồng sự kiện chờ tối đa 10 giây, quá hạn không tính vào SIM; mã HTTP đọc theo thao tác, 500 khi quay số tính vào SIM; phát hiện mới K-69…K-73 |
| `K-64` | 🆕 (tìm thấy 26/09 lúc làm K-61) | Giám sát. Panel trễ hạn trên dashboard vẫn cộng mọi lý do và vẫn ghi rằng lớn hơn 0 là mô hình dung lượng sai → tách theo `ivr_reason_code` như cảnh báo của `K-61`. Eligibility hỏng một phần (có đơn được xét, có đơn không) chưa có cảnh báo, vì counter không có lý do riêng cho đơn chưa được xét → thêm lý do ở sweep, như `Q-22.2` đã làm cho giờ gọi | dashboard Grafana, `PostgresSchedulerStore.cs`, `ivr-slo.rules.yml`, `capacity-selftest.mjs` | self-test dashboard; promtool | 1 | ✅ 5c5b820 · 1476/1476 (W-0372) · IT-SCH-UNEVALUATED-01, UT-DASH-REASON-06, IT-SLO-CAPACITY-04; lý do ELIGIBILITY_NOT_EVALUATED_BEFORE_DEADLINE, cảnh báo IvrConfirmationWindowsExpiringUnevaluated thay luật của K-61, panel tách theo lý do |
| `K-65` | 🆕 (tìm thấy 26/09 lúc làm K-59) | Tài liệu lệch: `specs/functional/05` dòng 46 còn câu về timeout `IVR_NO_ANSWER_FINAL` từ trước C7; IR-06 (quanh dòng 654 và 675) nói MOCK không có callback cho khách thật, nhưng sandbox MOCK có phát callback cho job dry-run, nay gồm cả job chưa được eligibility xét | `specs/functional/05-result-normalization-callback.md`, IR-06 (ghim lại) | các gate ghim hash | 0,5 | ✅ 5c5b820 · 1476/1476 (W-0372) · F05 theo C7 và C22; IR-06 §3.9, IR-07: DRY_RUN có callback, TASK_HELD_* ở intake phải gửi lại; ghim lại 4 validator, 3 template; báo M3 là việc của Toàn |
| `K-66` | 🆕 (tìm thấy 26/09 lúc làm K-65) | Tài liệu gửi M3 còn lệch: `IR-08` dòng 111 vẫn ghi task POST trả `200 TASK_HELD_ADMIN_REVIEW` lý do `ELIGIBILITY_SOURCE_VERSION_MISSING` (lỗi `K-59` đã sửa ở IR-06: intake trả `TASK_ACCEPTED_DRY_RUN_ONLY`, việc giữ ở bước eligibility rồi có callback hết cửa sổ); `IR-08` §6 không nói `task_id` không có kịch bản riêng thì về `IVR_CONFIRMED`; IR-06 §3.11 nói decision lệch vẫn nhận là `TASK_ACCEPTED_CALL_JOB_CREATED`, chỉ đúng ở LAB (sandbox trả `DRY_RUN_ONLY`, production trả `TASK_HELD_ADMIN_REVIEW`); IR-07 dòng 186 nói hai trong năm decision `200` không tạo cuộc gọi, thật ra là ba | `integration-requirements/08-module-3-sandbox-guide.md`, IR-06 (ghim lại), IR-07 | các gate ghim hash | 0,5 | ⬜ |
| `K-67` | 🆕 (tìm thấy 26/09 lúc làm K-65) | Intake: `attempt_policy_version` dài hơn 120 ký tự không bị schema chặn (`ivr-order-confirmation.v1.yaml` quanh dòng 1364); `PolicyVersion.Create` (`TaskIntakeService.cs` quanh dòng 130) ném lỗi không ai bắt, thành `500`, và Module 3 sẽ retry mãi. Sửa: bắt ở intake, trả `4xx` có mã ổn định như các lỗi hợp đồng khác, kèm IT; giới hạn trong schema là đổi hợp đồng nên để sau | `TaskIntakeService.cs`, test intake | IT intake với version quá dài | 0,5 | ⬜ |
| `K-68` | 🆕 (tìm thấy 26/09 lúc làm K-65) | Sandbox MOCK lệch production ở một kết cục: job dry-run không kịp quay đóng bằng `IVR_CONFIRMATION_WINDOW_EXPIRED` + giữ chờ duyệt, còn production đóng job xếp hàng không kịp quay bằng `IVR_CAPACITY_EXCEPTION` (`PostgresSchedulerStore.cs` quanh dòng 497, `IT-SCH-DEADLINE-12`). Module 3 vì vậy không thử được callback thiếu dung lượng trên sandbox. Sửa: cho job dry-run đi cùng nhánh thiếu dung lượng, hoặc ghi rõ trong `IR-08` | `PostgresSchedulerStore.cs` hoặc `IR-08` | IT sweep với job dry-run | 0,5 | ⬜ |
| `K-69` | 🆕 (tìm thấy 26/09 lúc làm K-63) | Gác máy lỗi sau khi đã có kết quả làm mất kết quả: ở `AsteriskSchedulerDispatchGateway.cs` quanh dòng 230, nếu lệnh gác máy sau disposition lỗi (từ `K-63` cả khi quá hạn 10 giây), lượt gọi bị ghi là lỗi kỹ thuật thay cho kết quả, kể cả khi khách đã bấm `1`, và khách bị gọi lại. Nặng nhất trong các phát hiện lần này. Sửa: ghi kết quả trước, lỗi gác máy ghi riêng | `AsteriskSchedulerDispatchGateway.cs`, test dispatch | UT dispatch: khách bấm `1`, gác máy lỗi hoặc quá hạn | 1 | ⬜ |
| `K-70` | 🆕 (tìm thấy 26/09 lúc làm K-63) | Kênh mồ côi ở adapter ARI: lệnh `DELETE` sau lệnh quay số quá hạn có thể tới Asterisk trước chính lệnh quay số chậm, kênh vẫn đổ chuông tới hết hạn quay; và khi worker tự huỷ giữa lúc quay, nhánh `catch` chung của `DialAsync` (quanh dòng 264) bỏ trạng thái mà không gác kênh. Khách bắt máy gặp một cuộc gọi im lặng, vì sự kiện của kênh lạ bị bỏ qua (`ProcessEvent`, quanh dòng 714). Sửa: nhớ id kênh đã bỏ và gác mọi kênh đó khi nó báo về; gác kênh cả khi huỷ | `AsteriskAriSimGateway.cs`, test Asterisk | UT: quay số chậm sau `DELETE`; huỷ giữa lúc quay | 1 | ⬜ |
| `K-71` | 🆕 (tìm thấy 26/09 lúc làm K-63) | Nhánh chung `_ =>` của dispatch gateway (`ASTERISK_DISPATCH_TECHNICAL_FAILURE`, kênh hỏng; `AsteriskSchedulerDispatchGateway.cs` quanh dòng 291) vẫn tính vào SIM các lỗi không phải của SIM: lỗi database từ sổ dial-token (trước khi quay) và từ store (sau khi quay), `ObjectDisposedException` lúc worker tắt. Một trả lời lỗi có charset lạ làm việc đọc body ném `InvalidOperationException`, rơi vào nhánh `ASTERISK_POLICY_OR_TOKEN_REJECTED` (quanh dòng 289). Sửa: nhánh riêng, sức khoẻ `null` cho lỗi phía IVR | `AsteriskSchedulerDispatchGateway.cs`, `AsteriskAriSimGateway.cs` | UT dispatch cho từng loại lỗi | 1 | ⬜ |
| `K-72` | 🆕 (tìm thấy 26/09 lúc làm K-63) | Khách bị tính lượt cho lỗi phía IVR: luồng sự kiện ARI không có keep-alive (quanh dòng 553, vòng nhận quanh dòng 646), nên luồng im lặng sau khi kết nối không bị phát hiện và cuộc đang mở kết thúc theo đồng hồ riêng, thành khách không nghe hoặc không bấm; sự kiện kết thúc phát không mang kênh nên bị bỏ, và lần phát hỏng về sau thành "khách không bấm". Sửa: keep-alive hoặc hạn im lặng cho luồng; gắn sự kiện phát với kênh qua `playbackId` | `AsteriskAriSimGateway.cs`, test Asterisk | UT: luồng im lặng giữa cuộc; phát hỏng giữa chừng | 1,5 | ⬜ |
| `K-73` | 🆕 (tìm thấy 26/09 lúc làm K-63) | Test double lệch adapter thật: `ScriptedAriSimGateway` và `IT-TEL-AUDIO-STATUS-11` coi `ASTERISK_PLAYBACK_FAILED` là lỗi âm thanh (`AudioError`, raw event `ERROR`), adapter thật báo lỗi mạng (raw event `NOT_PLAYED`). Chọn một bên đúng rồi sửa bên kia | `ScriptedAriSimGateway`, `MockTelephonyPersistenceTests.cs` hoặc `AsteriskAriSimGateway.cs` | `IT-TEL-AUDIO-STATUS-11` | 0,25 | ⬜ |
| `K-74` | 🆕 (tìm thấy 26/09 lúc làm K-64) | Giám sát: luật ngưỡng 0 trên counter bỏ sót lần tăng đầu tiên sau mỗi lần worker khởi động lại, vì OTel chỉ tạo series khi ghi lần đầu, bắt đầu từ 1, nên `increase()` không thấy bước 0 → 1. Gặp ở mọi luật như vậy trong `ivr-slo.rules.yml` (quanh dòng 69, 107, 211 và luật mới của `K-64`). Sửa: ghi sẵn mỗi series nhãn cố định bằng 0 lúc khởi động | `IvrTelemetry.cs` hoặc nơi đăng ký counter, `ivr-slo.rules.yml` | UT metric; promtool | 1 | ⬜ |
| `K-75` | 🆕 (tìm thấy 26/09 lúc làm K-64) | Tài liệu nội bộ lệch: `docs/operations/production-dial-path.md` dòng 296 vẫn đọc mọi giá trị khác 0 của `ivr_missed_deadline_total` là thiếu dung lượng; mô tả metric ở `IvrTelemetry.cs` dòng 211 (cửa sổ "đóng mà không có cuộc gọi") sai với lý do hết cửa sổ, vốn gồm cả khách đã được gọi | `docs/operations/production-dial-path.md`, `IvrTelemetry.cs` | không | 0,25 | ⬜ |
| `K-76` | 🆕 (tìm thấy 26/09 lúc làm K-64) | Sweep hết hạn không đọc `task.revoked_at`. Khi có endpoint thu hồi (`Q-20`), job bị thu hồi mà đang xếp hàng sẽ đóng thành thiếu dung lượng (mở incident và ticket), job chưa được xét sẽ đóng dưới lý do của `K-64`. Hôm nay chưa gì ghi `revoked_at` (`W-0249` README §8). Làm cùng `Q-20.1` | `PostgresSchedulerStore.cs` | IT sweep với task đã thu hồi | 0,5 | ⬜ |
| `K-77` | 🆕 (tìm thấy 26/09 lúc land K-63) | `generate-test-traceability.mjs` chỉ tìm tên method trong 12 dòng sau `[Trait("TestId")]`, nên theory có nhiều `InlineData` ra dòng trống tên method; C2 không khớp được kết quả của các TestId đó. Hôm nay có ba dòng như vậy: `UT-CALLBACK-STATE-08`, `UT-CALLBACK-TARGET-ACK-01`, `UT-VOICE-NUM-01`. `UT-AST-HTTP-04` của `K-63` gặp đúng lỗi này, đã xử lý bằng cách đặt `[Trait]` sát khai báo method. Sửa: đọc tới khai báo method thay vì 12 dòng, và cho gate đỏ khi tên method trống | `deploy/ci/scripts/generate-test-traceability.mjs`, `docs/traceability-tests.md` | self-test của generator | 0,25 | ⬜ |
...
| `Q-37` | Task đã nhận mà eligibility chặn: Module 3 có nhận callback không 🆕 | K-60 | chief (báo M3) | PA2 | 2 | ⏸ chief |
...
| Việc sau khi quyết | Giờ | Trạng thái |
| --- | ---: | --- |
| `Q-16.1` Theo PA2: render thử tại intake sau khi có `approvedScript`, lỗi thì từ chối với reason mới bằng decision và mã có sẵn; giới hạn số chỉ ghi trong description; đính chính IR-07 và bảng mã lỗi | 3 | ✅ 04ebece · 947/947 (W-0371) · UT-INTAKE-RENDER-01/02, IT-INTAKE-RENDER-01; intake dựng thử lời thoại, không được thì TASK_HELD_ADMIN_REVIEW + 422 IVR_PII_POLICY_VIOLATION, reason SPEECH_SUMMARY_NOT_RENDERABLE (Toàn chọn mã); contract draft.34; chief chưa duyệt |
...
#### `Q-37` — Task đã nhận mà eligibility chặn: Module 3 có nhận callback không 🆕 (`K-60`) · chief (báo M3: đổi tài liệu gửi M3)

IR-06 bảo Module 3 "chờ callback" khi intake trả `TASK_ACCEPTED_CALL_JOB_CREATED`, và hứa mọi task đã accepted đi tới một
kết cục có callback hoặc một incident kỹ thuật quan sát được. Eligibility chạy sau intake vẫn có thể chặn task
(`TASK_BLOCKED_OPERATIONAL`: token hết hạn, liên lạc không dùng được, cửa sổ chưa mở...). Job khi đó đóng ngay, không có
kết quả, không có callback, vì DT-06 không phát kết quả blocked. Module 3 chờ tới timeout của chính mình. `K-60`
(`W-0369`) đã bỏ phần tranh chấp: task tới eligibility khi cửa sổ đã hết luôn do sweep đóng, kèm callback. Phần còn
lại là hợp đồng, nên cần chief.

| | Phương án | Ưu | Nhược | Công |
| --- | --- | --- | --- | --- |
| PA1 | Phát callback ngay khi eligibility chặn: `IVR_TECHNICAL_EXCEPTION`, final, không tính lượt, `CORE_REVALIDATE_AND_HOLD_ADMIN_REVIEW`, mã lý do là mã chặn | Module 3 biết ngay; không thêm từ vựng trên dây | "Lỗi kỹ thuật" sai nghĩa với một chặn có chủ ý; Module 3 phải đọc mã lý do mới phân biệt được; đổi tài liệu gửi M3 | ~1 ngày |
| PA2 | Job bị chặn chưa đóng; sweep hết hạn đóng nó như `K-54`: `IVR_CONFIRMATION_WINDOW_EXPIRED`, không tính lượt, `CORE_REVALIDATE_AND_HOLD_ADMIN_REVIEW`, một callback | Không thêm nghĩa mới trên dây: cặp kết quả này Module 3 đã phải xử lý; một lối đóng duy nhất cho mọi task không gọi được | Module 3 biết muộn, tới hết cửa sổ; kết quả nói "hết cửa sổ" dù lý do là chặn (mã chặn nằm trong audit) | ~0,5 ngày |
| PA3 | Giữ im lặng, sửa IR-06: task bị chặn sau intake không có callback, Module 3 dùng timeout của mình | Không đổi code | Trái lời hứa hiện có trong IR-06; Module 3 phải dựng timeout; đơn treo lâu hơn | ~0,25 ngày tài liệu |

**Khuyến nghị: PA2.** Giữ nguyên nghĩa trên dây, và đi cùng một lối với `K-52`/`K-54`. Nếu Module 3 cần biết ngay thì
chọn PA1, và chief phải nói rõ `IVR_TECHNICAL_EXCEPTION` được dùng cho chặn có chủ ý.

| Việc sau khi quyết | Giờ | Trạng thái |
| --- | ---: | --- |
| `Q-37.1` Làm theo PA đã chọn: với PA2, job bị chặn không đóng ngay mà chờ sweep hết hạn đóng như `K-54`, kèm IT; sửa IR-06 (bảng decision intake và khối C22), ghim lại, báo M3 | 2 | ⬜ |
...
| `B16` | `total_amount` số lẻ, cách ly SIM | Một phần ✅; `Q-12.1` ✅ `8a5e6f7`, `Q-16.1` ✅ `04ebece` 26/09 | `K-14`, `K-26`, `K-27`, `K-29`, `K-30`, `Q-12`, `Q-16` |
...
| `C21` | Phát lại callback không giới hạn tuổi | Tài liệu ✅; code: `Q-19.1` ✅ `04ebece` 26/09 (7 ngày, `draft.34`) | `K-13`, `Q-19` |
