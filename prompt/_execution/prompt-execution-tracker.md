...
| `W-0061` | **GitLab platform provisioning** (TV1-12) | Platform/Infra | BLOCKED_EXTERNAL | project/dual remote, SaaS pipeline `#2756119982` và self-hosted pipeline `#2756183002` đều xanh; runner `#55115499` đã chứng minh tagged Linux-container + DIND; còn Container Registry, protected default branch, MR approvals, "Pipelines must succeed", Pages access control và masked/protected variables | Windows host + Docker Desktop Linux containers; runner tag `ginsengfood-docker`; không ghi token vào evidence | cấu hình/chụp evidence settings/protection/registry còn lại; không lặp lại phần runner đã PASS |
...
| `W-0011` | `P0-2` | GitLab CI/quality baseline | W-0010 ACCEPTED | TESTS_PASS | Codex | `.gitlab-ci.yml`; `deploy/ci/**`; MR template; CODEOWNERS; lockfiles; `docs/evidence/W-0011/`; `docs/evidence/W-0061/` | hosted self-hosted pipeline `#2756183002`: 9/9 jobs, 98/98 tests, coverage 91.5%, validate/build/test/security/privacy + DIND PASS | CI code và hosted runner proof hoàn tất; giữ TESTS_PASS vì protected branch/MR approvals/Pipelines-must-succeed/Registry/Pages/protected variables còn mở ở W-0061 |
