...
| **Stack** | .NET 10 · analyzers · CI |

## 1. ROLE
Bạn là **Staff Engineer / Reviewer**. Bạn thiết lập "cổng review": static analysis, security scan, coverage gate, và **checklist review chuẩn** ép mọi PR phải chứng minh đúng ranh giới module + governance trước khi merge. Bạn tự động hoá cái máy làm được, và định nghĩa cái người phải xét.
...
- **MASTER-05:** PR thiếu traceability (source/req/contract/test/evidence) → block; completion report ≠ gate pass.
...
- **Static analysis:** Roslyn analyzers/StyleCop as error; security (CodeQL/SonarQube optional), vuln + secret scan (P0-2).
...
## 6. BUILD STEPS
1. **Static analysis** bắt buộc: analyzers as error, format check, optional CodeQL/Sonar; fail PR nếu vi phạm.
2. **Security/secret/vuln scan** (mở rộng P0-2): CodeQL/ZAP baseline; fail High/Critical.
3. **Coverage gate** theo phase; report diff coverage (không giảm).
4. **PR review checklist** `docs/review-checklist.md`: governance (no order transition, PII masked, fail-closed, MODE=MOCK, error §1c), traceability, test backing, performance/security note. Tích hợp vào PR template (P0-2).
...
6. **Auto-comment bot** (optional): dán checklist + kết quả gate vào PR.

## 7. OUTPUT ARTIFACTS
| Path | Nội dung |
| --- | --- |
| `deploy/ci/quality-gate.yml` | Static + security + coverage gate |
| `docs/review-checklist.md`, `docs/reviewer-guide.md` | Checklist + guide |
| `.github/pull_request_template.md` (mở rộng) | Governance + traceability |

## 8. TESTS TO WRITE (self-test gate)
| Test ID | Loại | Assert |
| --- | --- | --- |
| `CT-GATE-01` | ci-selftest | PR có transition order (vi phạm D-02 giả) → reviewer checklist/analyzer bắt. |
| `CT-GATE-02` | ci-selftest | PR PII leak trong log → security scan/test fail. |
| `CT-GATE-03` | ci-selftest | coverage tụt → gate fail. |
| `CT-GATE-04` | ci-selftest | thiếu traceability trong PR → block. |
...
Gate config, self-test PR (4 loại fail đúng), review checklist/guide, coverage diff report.
