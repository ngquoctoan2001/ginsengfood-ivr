...
## 1. ROLE
Bạn là **Senior Security/Platform Engineer**. Bạn quản trị vòng đời secret/credential/key của IVR: rotation định kỳ và khẩn cấp, zero-downtime, và bảo vệ đặc biệt cho **dial-token/token-vault key** (D-05). Bạn đảm bảo lộ secret không thành thảm hoạ nhờ rotation + least-exposure.

## 2. CONTEXT
IVR giữ nhiều secret: service credential (Core/ops/CRM), DB creds, SIM gateway creds, token-vault key (map dial_token→số — nhạy cảm nhất, D-05). Không có lifecycle rotation = rủi ro lộ kéo dài. Prompt này xây rotation + KMS/Vault lifecycle + runbook.
...
- Secret store (Vault/KMS/K8s Secret — `NEED_CONFIRMATION` prod=Vault/KMS); service cred (P4-4); token vault (P2-4/P8-1).

## 6. BUILD STEPS
1. **Secret inventory + classification**: liệt kê secret, độ nhạy, chủ sở hữu, TTL rotation.
2. **Rotation mechanism**: dual-key overlap (đọc key mới+cũ trong window); rotate DB/service/SIM creds + token-vault key zero-downtime.
...
| `SEC-ROT-02` | security | rotate token-vault key không lộ số; SIM adapter tiếp tục hoạt động (D-05). |
...
**Self-review:** [ ] zero-downtime dual-key; [ ] token-vault key bảo vệ (D-05); [ ] emergency runbook; [ ] audit không lộ giá trị; [ ] least-exposure.
**Reviewer:** revoke đúng sau overlap; scope tối thiểu; propagation verify.

## 10. EVIDENCE EXPECTED
Rotation drill log (zero-downtime), token-vault rotation proof, emergency revoke test, secret-scan clean, rotation audit.
