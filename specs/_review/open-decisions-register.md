...
| `OD-V1-02` | generic callback path and ACK taxonomy | Sales API/Core | ⏳ `M8_POSITION_SIGNED / M3_NOT_RECEIVED` 2026-09-05 — ký Target V1: `POST /api/v1/internal/orders/{orderId}/ivr-result-callbacks`; ACK 200 `ACCEPTED`/`DUPLICATE_ACCEPTED`/`BLOCKED_BY_CORE`/`REVIEW_REQUIRED`, 409 `REJECTED_STALE`/`IDEMPOTENCY_CONFLICT`. Endpoint Golden Hour giữ vai trò compatibility-only | OpenAPI + contract tests |
| `OD-V1-03` | order version exposure/bump/stale behavior | Sales Core | ⏳ `M8_POSITION_SIGNED / M3_NOT_RECEIVED` 2026-09-05 — Sales phơi `order_version`; IVR đính bản nó thấy; Sales revalidate và tự quyết. Lệch phiên bản trả ACK `REJECTED_STALE` và **không** ghi đè kết quả khách đã bấm | DTO + stale tests |
