...
| Giờ gọi | Có attempt (`T0 + offset`, `T0` là `confirmation_window_started_at`) rơi ngoài giờ gọi `08:00–21:08` (giờ VN): `T0` trước `08:00`, hoặc `T0` từ `21:00:30` (`TWENTY_FOUR_SEVEN`) / `21:05:30` (`GOLDEN_HOUR`) — *từ `26/09` (`Q-22`); `25/09` (`B17`): `T0` nằm ngoài giờ gọi; trước đó (`W-0298`, `16/09`): không attempt nào của `attempt_policy` rơi vào giờ gọi, trên toàn bộ confirmation window* | `TASK_BLOCKED_OPERATIONAL` | `CALLING_WINDOW_CLOSED_FOR_WHOLE_CONFIRMATION_WINDOW` (giữ tên dù điều kiện đổi) | `200` kèm `decision` và `blocked_reasons` — M3 **đọc được** reason này. Task **không** được lưu. **Không** retry trong cùng cửa sổ: kết quả không đổi. M3 giữ đơn `TWENTY_FOUR_SEVEN` (COD) phát sinh ngoài `08:00–21:00:30`, rồi gửi lại từ `08:00` với cửa sổ mới và `Idempotency-Key` mới; được dùng lại `task_id` (`IR-07`, đính chính `25/09`). *Thêm `25/09`, `W-0354`; sửa `25/09` (`B17`): điều kiện theo `T0`, và "gửi task mới" thành "gửi lại, được dùng lại `task_id`"* |
...
| Lời thoại | tóm tắt đơn qua mọi guard theo từng field nhưng không dựng được thành lời thoại theo kịch bản đã duyệt: `total_amount` lớn hơn `999999999999`, lời thoại quá `1.200` ký tự, hoặc một giá trị mà guard lời nói từ chối. Intake dựng thử bằng đúng bộ dựng lời thoại của lúc quay — *thêm `26/09` (`Q-16`, `1.0.0-draft.34`); trước đó intake nhận rồi mọi lần quay đều hỏng tới hết cửa sổ* | `TASK_HELD_ADMIN_REVIEW` | `SPEECH_SUMMARY_NOT_RENDERABLE` | `422 IVR_PII_POLICY_VIOLATION`, cùng mã với dòng Privacy ngay trên |
